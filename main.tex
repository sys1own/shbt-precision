...
% Theorem-like environments used in the supplementary material
\theoremstyle{plain}
...
answers the second with an active Stinespring isometric state-projection map that strips visible color and
...
topological anti-baryon state decoupling.  This section records the corresponding finite audit
...
We model the unresolved excess as a congestion number
$\Delta N = N_{\rm local}-N_{\rm limit}$, where $N_{\rm local}$ is the
information count in the congested patch and $N_{\rm limit}$ is the
...
M_{\rm seed} = \alpha_{\rm seed}\,(N_{\rm local}-N_{\rm limit}),
\end{equation}
with coupling coefficient
\begin{equation}
\alpha_{\rm seed} \approx 1.67\times10^{-51}\;M_{\odot}\;{\rm bit}^{-1}.
\end{equation}
...
\begin{equation}
M_{\rm seed}
= (1.67\times10^{-51}\;M_{\odot}\;{\rm bit}^{-1})
(6\times10^{59}\;{\rm bits})
\approx 10^{9}\,M_{\odot}.
\end{equation}
...
\midrule
Initial gas entropy & $S_{\rm gas}$ & $1.1\times10^{45}$ J/K & thermal reservoir before derendering \\
...
High-energy transient benchmark & $P_{\rm bench}$ & $1.4208\times10^{8}$ W ($142.08$ MW) & calibration transient \\
Transient power ratio & $\Gamma_{\rm bench}$ & $\dot Q/P_{\rm bench}\approx 6{,}377$ & signal excess over benchmark \\
\bottomrule
\end{tabularx}
...
$\Gamma_{\rm bench}$ quantifies the excess over the calibrated
...
$\Gamma_{\rm bench}\approx 6{,}377$ shows that the congestion burst is a
...
C_{\rm even}(\alpha_1,\alpha_2,\alpha_3)
=
\bigl[\pi\mu\,\gamma(2b^{2}+1)\,b^{1-b^{2}}\bigr]^{\,bQ-\sum_i\alpha_i}
\frac{
\Upsilon_{\rm NS}\!\left(\sum_i\alpha_i-Q\right)
\prod\limits_{\text{cyc}} \Upsilon_{\rm NS}\!(\alpha_i+\alpha_j-\alpha_k)
}{
\Upsilon'_{\rm NS}(0)
\prod\limits_{i} \Upsilon_{\rm NS}\!(2\alpha_i)
},
\end{equation}
where $Q=b+b^{-1}$ is the background charge, $\mu$ is the cosmological
coupling, $\gamma(x)=\Gamma(x)/\Gamma(1-x)$, and $\Upsilon_{\rm NS}$ is the
Neveu-Schwarz \(\Upsilon\)-function.  The odd-spin counterpart $C_{\rm odd}$
...
\frac{\partial}{\partial\tau}\,\ln C_{\rm even/odd} = 0.
\end{equation}
...
$M_{\rm seed}$.  Combined with the completed central-charge ledger
$\cdarkcomp$ and the zero-framing condition $\Delta_{\rm fr}=0$ from
...
superconformal seed stabilization are all
read as
...
\pyvar{report.stress_energy_preserved=True}: the Stinespring isometric state-projection map is allowed only
...
asymmetry produced by the state-decoupling loop, with the benchmark audit giving
...
cosmology calculations are maintained in the GitHub repository\footnote{\url{https://github.com/sys1own/shbt-precision.git}}.  During
