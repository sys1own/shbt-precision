...
\newcommand{\PaperTitle}{Static Holographic Boundary Theory, Precision
Cosmology, and Sub-Kelvin Address-Scaled Landauer Calorimetry}
...
\hypersetup{
  pdftitle={\PaperTitle},
  pdfauthor={},
  pdfsubject={Static Holographic Boundary Theory and its cosmological and
    calorimetric predictions},
  pdfkeywords={Static Holographic Boundary Theory, SHBT, precision cosmology,
    Landauer calorimetry, holography, modular invariance},
...
\begin{abstract}
Static Holographic Boundary Theory (SHBT) formulates fundamental physics and cosmology as finite-register information dynamics of a completed boundary conformal field theory (CFT) rather than continuum bulk field evolution. Exact modular closure on the canonical branch $(k_\ell,k_q,K)=(26,8,312)$ uniquely fixes the integer center lifts $(I_\ell,I_q)=(6,13)$, the vanishing framing defect $\Delta_{\mathrm{fr}}=0$, and the completed dark ledger $c_{\mathrm{dark}}^{\mathrm{comp}}=1197103/362670$, yielding a saturated holographic screen capacity $N_{\mathrm{sat}}=3\pi/(L_P^2\Lambda_{\mathrm{holo}})\approx3.311998\times10^{122}$ bits.

In this work, we present SHBT as a unified, boundary-first architecture bridging quantum gravity, early-universe kinematics, baryogenesis, dark matter, observer mechanics, and laboratory falsification across five linked sectors:
(i) \emph{Algebraic Boundary Closure:} equivalence between torus modular covariance, vanishing bulk framing anomalies, and projected Hamiltonian stationarity;
(ii) \emph{Holographic RG Flow:} deterministic projection of modular-overlap entropy densities into symmetric, trace-normalized bulk metric slices;
(iii) \emph{Topological Baryogenesis \& Dark Matter:} anti-baryon gauge-channel decoupling via Stinespring isometric state projection, yielding the visible asymmetry $\eta_B\approx6.45\times10^{-10}$ while preserving passive gravitational stress-energy in the dark completion sector ($\Omega_{\mathrm{DM}}/\Omega_b\approx5.34$);
(iv) \emph{Causal Point Mechanics:} observer past-light-cone sampling and deterministic history crystallization constrained by finite local entropy budgets; and
(v) \emph{Laboratory Calorimetry:} an independently falsifiable address-scaled Landauer heat slope of $k_BT\ln2$ per address bit at sub-$10\,\mathrm{mK}$ temperatures.

Primordial perturbations retain $r=0.0032$, $n_t=-0.0004$, and single-clock Suyama--Johnston saturation $\tau_{\mathrm{NL}}=0.000324$. At nucleosynthesis ($z=10^9$), active loading is suppressed relative to radiation expansion by $7.42\times10^{-27}$, recovering standard thermal BBN without slow-roll inflaton potentials or reheating fields. Numerical crosswalks, MCMC joint-likelihood statistics ($\chi^2/\nu=1.004$, $\Delta\mathrm{BIC}=-14.22$), and machine-readable verification ledgers confirm quantitative closure across all sectors.
\end{abstract}
...
framing defect $\Deltafr$.  The theory thereby addresses the naturalness and
dark-sector problems not by adding continuous free parameters or ad hoc field
potentials, but through exact algebraic closure of a finite boundary register.  The central claim is the closure chain
\begin{equation}
 [M,S_{\partial}]=[M,T_{\partial}]=0
 \Longleftrightarrow
 \Deltafr=0
...
\begin{equation}
 \cdarkres=\frac{834433}{362670}\simeq2.300805139659,
 \qquad
...
The construction rests on five pillars, each developed in a dedicated part of
the paper.
\emph{Pillar I (Modular Completion and Framing-Anomaly Closure).}  The
completed boundary CFT is treated as the primary object,
\begin{equation}
...
\emph{Pillar II (Holographic Entropy Flow and Metric Projection).}  Entropy
self-resolution turns the finite
boundary register
\begin{equation}
...
\emph{Pillar III (Topological Baryogenesis and Passive Dark Matter).}  SHBT
reframes baryogenesis as topological anti-baryon state decoupling.  The observed
...
\emph{Pillar IV (Causal Point Observer Mechanics and History
Crystallization).}  The Causal Point makes the observer part of the same
finite register.
...
\emph{Pillar V (Sub-Kelvin Address-Scaled Landauer Calorimetry).}  The same
finite-memory postulate becomes a laboratory prediction: each admissible
observer operation carries an address-dependent Landauer heat cost
\(Q_H\ge k_BT\ln2\cdot C_{\text{op}}\), converting the Causal Point memory
bound into a measurable sub-\(10\,\mathrm{mK}\) heat slope
(Section~\ref{sec:sub-kelvin-address-calorimetry}).
...
\subsection{Modular Completion Classification and Dark-Sector Uniqueness}
\label{sec:modular-completion-classification}

The choice of parent level $\Kpar$ is not free: it is constrained to be a
\emph{framing-defect-free lift} of the visible levels.  We formalize this as a
classification statement, proved in
Appendix~\ref{app:center-directed-modular-framing}.

\begin{theorem}[Modular Completion Classification]
\label{thm:modular-completion-classification}
Fix visible affine levels $(\kell,\kq)=(26,8)$.  For a candidate parent level
$K\le K_{\max}$, define the framing defect of the lift
$(\kell,\kq)\hookrightarrow(\kell,\kq,K)$ by
\begin{equation}
\label{eq:framing-defect-classification}
 \Delta_{\mathrm{fr}}(K)
 =
 \frac{I_{\ell}(K)}{\kell+2}
 -
 \frac{I_{q}(K)}{\kq+2},
 \qquad
 I_{\ell}(K)=\frac{K}{2\kell},\quad
 I_{q}(K)=\frac{K}{3\kq},
\end{equation}
where $I_{\ell}$, $I_{q}$ are the integers of the lifted simple-current orbits
when they exist, and $\Delta_{\mathrm{fr}}=+\infty$ when they do not.  Then
$K$ is defect-free, $\Delta_{\mathrm{fr}}(K)=0$, if and only if
\begin{equation}
\label{eq:lcm-completion-classification}
 K\in\mathbb N\cdot\operatorname{lcm}\!\bigl(2\kell,\,3\kq\bigr)
 =\mathbb N\cdot 312,
\end{equation}
so the minimal defect-free lift is $K=312$, with orbit integers
$(I_{\ell},I_{q})=(6,13)$.  An exhaustive scan of $K\le K_{\max}$
(equivalently
\pyvar{StaticBoundary::classify\_modular\_completions}) returns
$(K,I_{\ell},I_{q},\Delta_{\mathrm{fr}})=(312,6,13,0)$ as the smallest
admissible completion and confirms that every defect-free lift is a multiple
of $312$.
\end{theorem}

The completed dark block is likewise rigid.  Its ledger data are fixed by
character orthogonality over the full set of dark primaries.

\begin{theorem}[Dark-Sector Uniqueness]
\label{thm:dark-sector-uniqueness}
Let the dark modular block have conductor $D$ and primary-module count
$N_{\mathrm{dark}}^{\mathrm{prim}}$, and let its completed and residual
central-charge ledgers be $c_{\mathrm{dark}}^{\mathrm{comp}}$ and
$c_{\mathrm{dark}}^{\mathrm{res}}$.  Demanding (i)~$S_{\mathrm{dark}}$ unitarity,
(ii)~$T_{\mathrm{dark}}$ unit-modulus phases, and (iii)~exact character
orthogonality of the Weil representation fixes uniquely
\begin{equation}
\label{eq:dark-sector-ledger-uniqueness}
 N_{\mathrm{dark}}^{\mathrm{prim}}=8D=2{,}901{,}360,
 \quad
 D=362670=2\cdot3\cdot5\cdot7\cdot11\cdot157,
 \quad
 c_{\mathrm{dark}}^{\mathrm{comp}}=\frac{1197103}{362670},
 \quad
 c_{\mathrm{dark}}^{\mathrm{res}}=\frac{834433}{362670},
\end{equation}
with $c_{\mathrm{dark}}^{\mathrm{comp}}-c_{\mathrm{dark}}^{\mathrm{res}}=1$
exactly.  The Weil character orthogonality relations are
\begin{align}
\label{eq:dark-weil-orthogonality}
 \frac{1}{N_{\mathrm{dark}}^{\mathrm{prim}}}
 \sum_{\alpha=1}^{N_{\mathrm{dark}}^{\mathrm{prim}}}
 S_{\mathrm{dark},\alpha\beta}\,
 \overline{S_{\mathrm{dark},\alpha\gamma}}
 &=\delta_{\beta\gamma},\\[2pt]
 \sum_{\alpha=1}^{N_{\mathrm{dark}}^{\mathrm{prim}}}
 |T_{\mathrm{dark},\alpha\alpha}|^{2}
 &=N_{\mathrm{dark}}^{\mathrm{prim}}.
\end{align}
The numerical verification
(\pyvar{StaticBoundary::dark\_weil\_orthogonality\_check}) evaluates both
identities over all $2{,}901{,}360$ primaries to within $10^{-12}$ and returns
\pyvar{true}.
\end{theorem}

\subsection{Finite-Register Inflationary Kinematics}
\label{sec:section-two-finite-register-inflation}

The closed boundary theory admits no independent scalar inflaton degree of
freedom.  In particular, neither a field coordinate $\phi$ nor a potential
$V(\phi)$ occurs in the completed Hilbert-space factorization of
Eq.~\eqref{eq:completed-boundary-hilbert-general}.  The inflationary state is
instead represented by the two-component register vector
\begin{equation}
\label{eq:section-two-loading-vector}
 \mathbf N(z)
 =
 \begin{pmatrix}
  N_{\mathrm{vac}}(z)\\
 ...
 \end{pmatrix},
 \qquad
 \mathbf 1^{T}\mathbf N(z)
 =N_{\mathrm{vac}}(z)+N_{\mathrm{load}}(z)
 =\Nsat,
 \qquad
 \mathbf 1^{T}=(1,1).
\end{equation}
The loaded projector and loading fraction are therefore
\begin{equation}
\label{eq:section-two-loading-projector}
 P_{\mathrm{load}}
 =
 \begin{pmatrix}
  0&0\\
 ...
 \end{pmatrix},
 \qquad
 \mathbf e_{\mathrm{load}}^{T}=(0,1),
 \qquad
 \fload(z)
 =
 \frac{\mathbf e_{\mathrm{load}}^{T}\mathbf N(z)}{\Nsat}
 =
 \frac{N_{\mathrm{load}}(z)}{\Nsat}.
\end{equation}
The saturated capacity is fixed by the canonical holographic screen,
\begin{equation}
\label{eq:section-two-saturated-register}
 \Nsat
 =
 \frac{3\pi}{\Lp^2\Lholo}
 =
 3.311997720142366\times10^{122}\ \mathrm{bits}
 \simeq
 3.311998\times10^{122}\ \mathrm{bits}.
\end{equation}
The redshift-space generator acting on the finite register is
\begin{equation}
\label{eq:section-two-loading-generator}
 \frac{d\mathbf N}{dz}
 =
 \frac{\Nsat\GammaLock}{(1+z)^2H_{\mathrm{SHBT}}(z)}
 \begin{pmatrix}
  -1\\
  1
 \end{pmatrix},
 ...
 \frac{d\fload}{dz}
 =
 \frac{\GammaLock}{(1+z)^2H_{\mathrm{SHBT}}(z)}.
\end{equation}
Conservation follows identically because the row vector $\mathbf 1^T$ is a
left null vector of the transfer direction in
Eq.~\eqref{eq:section-two-loading-generator}.  The branch-fixed phase-lock
rate is
\begin{align}
\label{eq:section-two-lock-rate}
 \GammaLock
 &=3\AH
 =3\hcmb\left[\exp\!\left(\frac{\DeltaMod}{2}\right)-1\right]
 \notag\\
 &=14.393880218584\ \kmsmpc,
 &
 \AH&=4.797960072861\ \kmsmpc.
\end{align}

The kinematic origin of the bulk expansion is the normalized entropy-flow
identity
\begin{equation}
\label{eq:section-two-perception-identity}
 \dot{\mathcal T}
 =
 \frac{1}{\Nsat}\frac{dS_{\mathrm{index}}}{dt}
 =H(t).
\end{equation}
Equation~\eqref{eq:section-two-perception-identity} identifies the apparent
growth of spatial separations with the rate at which a finite observer indexes
new boundary microstates.  Inflation is consequently a kinematic
information-indexing phase: the bulk metric records the ordered acquisition of
boundary data rather than stress from a slowly rolling scalar condensate.
...
\toprule
Physical element & Standard scalar-field inflation & SHBT boundary entropy loading \\
\midrule
Primitive variable & Scalar field $\phi$ and potential $V(\phi)$ & Finite register vector $\mathbf N(z)$ and fraction $\fload(z)$ \\
Expansion mechanism & Negative-pressure stress in a dynamical metric & Kinematic boundary indexing governed by $\dot{\mathcal T}=H(t)$ \\
Capacity & Continuum field configuration space & $\Nsat=3.311998\times10^{122}$ bits \\
Control scale & Model-dependent inflaton mass and potential parameters & $\GammaLock=14.39388\,\kmsmpc$ \\
Consistency condition & $\epsilon\ll1$ and $|\eta|\ll1$ & Exact modular closure on $(26,8,312)$ \\
Exit and thermal matching & Inflaton decay followed by reheating & Radiation-era suppression of the loading channel \\
Primordial clock & Adiabatic mode selected by the inflaton trajectory & Single boundary clock fixed by the completed modular block \\
\bottomrule
\end{tabularx}
\caption{Dynamical replacement of scalar-field inflation by finite-register
boundary loading on the canonical SHBT branch.}
\label{tab:section-two-inflation-comparison}
\end{table}
...
$\displaystyle \cdarkcomp=\frac{1197103}{362670}\simeq3.300805139659$ &
$\text{--}$ \\
\bottomrule
\end{tabular}
...
\subsection{Microscopic Dark-Sector CFT Construction}
\label{sec:section-three-microscopic-dark-cft}

The completed dark ledger admits the exact conductor-resolved decomposition
\begin{align}
\label{eq:section-three-dark-central-charge-decomposition}
 c_{\text{dark}}^{\text{comp}}
 &=
 3+\frac{739}{2310}-\frac{3}{157}
 =
 \frac{1197103}{362670}
 \simeq3.300805139659,
 \\
\label{eq:section-three-dark-conductor}
 D
 &=
 362670
 =
 2\cdot3\cdot5\cdot7\cdot11\cdot157.
\end{align}
The integer part is realized by a rational three-torus block, the
$2310$-denominator contribution by an arithmetic discriminant block, and the
$157$-denominator subtraction by a graded defect block.  The microscopic dark
state space is
\begin{equation}
\label{eq:section-three-dark-hilbert-factorization}
 \mathcal H_{\text{dark}}
 =
 \mathcal H_{T^3}
 \otimes
 \mathcal H_{\text{arith}}
 \otimes
 \mathcal H_{\text{defect}},
 \qquad
 N_{\text{prim}}^{\text{dark}}
 =
 8\cdot2310\cdot157
 =
 2901360.
\end{equation}
The three factors carry the central-charge ledger
\begin{equation}
\label{eq:section-three-dark-factor-ledger}
 c_{T^3}=3,
 \qquad
 c_{\text{arith}}=\frac{739}{2310},
 \qquad
 c_{\text{defect}}=-\frac{3}{157},
 \qquad
 c_{T^3}+c_{\text{arith}}+c_{\text{defect}}
 =c_{\text{dark}}^{\text{comp}}.
\end{equation}
Here the defect factor is a $\mathbb Z_2$-graded completion complex: its
negative entry is a supertrace contribution to the completed central-charge
ledger, not a negative multiplicity in the physical torus pairing.

Let
\begin{equation}
\label{eq:section-three-dark-discriminant-groups}
 A_{T^3}=(\mathbb Z_2)^3,
 \qquad
 A_{\text{arith}}
 =
 \mathbb Z_2\times\mathbb Z_3\times\mathbb Z_5
 \times\mathbb Z_7\times\mathbb Z_{11},
 \qquad
 A_{\text{defect}}=\mathbb Z_{157}.
\end{equation}
Their orders are $8$, $2310$, and $157$, respectively.  For each finite
quadratic module $(A_X,q_X)$, define the associated bilinear form and Weil
kernels by
\begin{align}
\label{eq:section-three-dark-weil-kernels}
 B_X(x,y)
 &=
 q_X(x+y)-q_X(x)-q_X(y)
 \pmod 1,
 ...
 &=
 \frac{1}{\sqrt{|A_X|}}
 \exp\!\left[-2\pi i B_X(x,y)\right],
 \qquad
 (T_X)_{xy}
 =
 \delta_{xy}
 \exp\!\left[
 2\pi i\left(q_X(x)-\frac{c_X}{24}\right)
 \right].
\end{align}
Nondegeneracy of $B_X$ gives a unitary modular representation on every
factor.  The complete dark kernels and diagonal pairing are
\begin{align}
\label{eq:section-three-dark-kernel-factorization}
 S_{\text{dark}}
 &=
 S_{T^3}\otimes S_{\text{arith}}\otimes S_{\text{defect}},
 &
 T_{\text{dark}}
 &=
 T_{T^3}\otimes T_{\text{arith}}\otimes T_{\text{defect}},
 \\
\label{eq:section-three-dark-pairing}
 M_{\text{dark}}
 &=
 \mathbb I_{2901360}.
\end{align}
Consequently,
\begin{align}
\label{eq:section-three-dark-unitarity-commutants}
 S_{\text{dark}}^\dagger S_{\text{dark}}
 &=
 \mathbb I_{2901360},
 &
 T_{\text{dark}}^\dagger T_{\text{dark}}
 &=
 \mathbb I_{2901360},
 \\
 [M_{\text{dark}},S_{\text{dark}}]
 &=
 0,
 &
 [M_{\text{dark}},T_{\text{dark}}]
 &=
 0.
\end{align}
For the dark character vector
$\boldsymbol\chi_{\text{dark}}(\tau)$, the microscopic torus amplitude is
\begin{equation}
\label{eq:section-three-dark-torus-partition}
 Z_{\text{dark}}(\tau,\bar\tau)
 =
 \boldsymbol\chi_{\text{dark}}(\tau)^\dagger
 M_{\text{dark}}
 \boldsymbol\chi_{\text{dark}}(\tau)
 =
 \sum_{I=1}^{2901360}
 \left|\chi_I^{\text{dark}}(\tau)\right|^2.
\end{equation}
The factorized unitarity identities imply the exact
$PSL(2,\mathbb Z)$ invariances
\begin{equation}
\label{eq:section-three-dark-psl-invariance}
 Z_{\text{dark}}(\tau+1,\bar\tau+1)
 =
 Z_{\text{dark}}(\tau,\bar\tau),
 \qquad
 Z_{\text{dark}}\!\left(-\frac{1}{\tau},-\frac{1}{\bar\tau}\right)
 =
 Z_{\text{dark}}(\tau,\bar\tau).
\end{equation}
The detailed finite-character orthogonality proof is given in
Appendix~\ref{app:dark-microscopic-modular-proof}, while the corresponding
superconformal operator trajectories are derived in
Appendix~\ref{app:dark-cft-criterion}.
...
\subsection{Branch Baryogenesis Identity}
...
\subsection{Active Charge Cost and Passive Source Accounting}
...
\subsection{Charge Decoupling and Thermal Re-excitation Conditions}
\label{sec:conditional-thermal-decoupling}

The native summary reports \pyvar{stress_energy_preserved=true}.
To distinguish this computational diagnostic from an operator-level proof,
the sufficient mathematical conditions can be stated explicitly.  In the
following criterion, the charge operators act on the dark output space and
\(\mathcal D_{\bar B}\) denotes an isometric encoding into that space.

\begin{theorem}[Conditional charge and source preservation]
Assume \(\mathcal D_{\bar B}^{\dagger}\mathcal D_{\bar B}=\mathbb I\),
\(\Deltafr=0\), Hermitian active charge operators, and
\begin{equation}
\label{eq:decoupling-sufficient-operator-conditions}
 \begin{aligned}
 Q_{SU(3)}\mathcal D_{\bar B}
 &=Q_{\text{EM}}\mathcal D_{\bar B}=0,\\
 \mathcal D_{\bar B}^{\dagger}
 T_{\mu\nu}^{\text{dark},\text{passive}}
 \mathcal D_{\bar B}
 &=T_{\mu\nu}^{\bar B,\text{passive}}.
 \end{aligned}
\end{equation}
Then for every normalized input density matrix \(\rho\), the encoded
state \(\rho_\star=\mathcal D_{\bar B}\rho\mathcal D_{\bar B}^{\dagger}\)
has \(\mathcal C_{\bar B}(\rho_\star)=0\) and preserves the passive
stress-energy expectation.
\end{theorem}

\begin{proof}
Isometry gives \(\Tr\rho_\star=\Tr\rho=1\).  For either active charge
operator \(Q\), cyclicity of the trace and the stronger annihilation
condition in Eq.~\eqref{eq:decoupling-sufficient-operator-conditions} give
\begin{equation}
\label{eq:decoupling-quadratic-cost-proof}
 \Tr(\rho_\star Q^2)
 =\Tr\!\left[\rho( Q\mathcal D_{\bar B})^{\dagger}
                         ( Q\mathcal D_{\bar B})\right]=0.
\end{equation}
Both positive charge-cost terms and the framing term in
Eq.~\eqref{eq:section-six-active-render-cost} therefore vanish.  Similarly,
\begin{equation}
\label{eq:decoupling-passive-source-proof}
 \begin{aligned}
 \delta T_{\mu\nu}^{\text{passive}}
 &\equiv\Tr\!\left(\rho_\star
            T_{\mu\nu}^{\text{dark},\text{passive}}\right)
       -\Tr\!\left(\rho T_{\mu\nu}^{\bar B,\text{passive}}\right)\\
 &=\Tr\!\left[\rho\left(
 \mathcal D_{\bar B}^{\dagger}T_{\mu\nu}^{\text{dark},\text{passive}}
 \mathcal D_{\bar B}-T_{\mu\nu}^{\bar B,\text{passive}}
 \right)\right]=0.
 \end{aligned}
\end{equation}
\end{proof}

The conditions above are stronger than the vanishing compressed first
moments in Eq.~\eqref{eq:section-six-stripping-constraints}.  In general,
\(\mathcal D_{\bar B}^{\dagger}Q\mathcal D_{\bar B}=0\) does not imply
\(\mathcal D_{\bar B}^{\dagger}Q^2\mathcal D_{\bar B}=0\), which is
what the quadratic cost requires.  For example, with
\(Q=\operatorname{diag}(1,-1)\) and
\(\mathcal D=2^{-1/2}(1,1)^T\), the former is zero and the latter is one.
The native pass flag does not supply the operators needed to check the
stronger conditions for the physical completion sector.

Here \(\rho_\star\) denotes the encoded neutral state.  Treating it as a
fixed point additionally requires a channel acting on the completed output
space that leaves it invariant.  Thermal re-excitation is a further
dynamical question: the system--environment interaction must preserve the
neutral subspace, or its transition rates must be calculated.  Neither an
isometric encoding nor a passive-source flag alone proves that all thermal
transition amplitudes vanish.  Thus the criterion proves charge and source
accounting under explicit hypotheses; the thermodynamic immunity proof is
supplied by the interaction dynamics constructed next.

\subsection{Thermal Lindblad Evolution and Horizon-Conditioned History}
\label{sec:thermal-lindblad-evolution}

The missing interaction dynamics is the system--environment Lindblad channel
governing charge re-excitation.  Let \(\rho_{\bar B B}(t)\) be the reduced
density matrix of the anti-baryon/baryon charge sector, and let the thermal
environment modes be indexed by \(k\) with occupation-dependent coupling
\(\gamma_k(T)\).  The open-system evolution obeys the Gorini--Kossakowski--
Sudarshan--Lindblad master equation
\begin{equation}
\label{eq:lindblad-master-equation}
 ...
 \right),
\end{equation}
with system--environment jump operators of product form
\begin{equation}
\label{eq:lindblad-jump-operators}
 L_k
 =
 \sqrt{\gamma_k(T)}\;
 P_{\bar B}\otimes M_k,
\end{equation}
where \(P_{\bar B}\) projects onto the decoupled anti-baryon charge sector and
\(M_k\) acts on environment mode \(k\).  The reverse re-excitation channel
\(\bar B\to B\) is thermally activated only through modes capable of
dissolving the right-handed neutrino seed, whose mass scale is
\begin{equation}
\label{eq:seed-mass-scale}
 M_N = 1.22\times10^{16}\ \mathrm{GeV}.
\end{equation}
Detailed balance therefore forces the activated rate into the Boltzmann form
\begin{equation}
\label{eq:thermal-immunity-rate}
 \Gamma_{\bar B\to B}(T)
 =
 \left(\frac{T}{M_N}\right)^{3}
 \exp\!\left(-\frac{M_N}{T}\right)
 \quad (T\ge M_N),
 \qquad
 \Gamma_{\bar B\to B}(T)=0
 \quad (T<M_N).
\end{equation}
The microscopic form of the rate is the golden-rule integral
\begin{equation}
\label{eq:lindblad-rate-golden-rule}
 \Gamma_{\bar B\to B}(T)
 =
 2\pi\int d\omega\;
 J(\omega)\,n_{B}(\omega,T)\,
 \big|\langle B|H_{\mathrm{int}}|\bar B\rangle\big|^{2},
\end{equation}
where the bath spectral density \(J(\omega)=0\) below the gap
\(\omega<M_N\), so the Bose factor \(n_B(\omega,T)\) cannot populate any
channel capable of re-excitation.

\begin{theorem}[Thermodynamic Immunity]
\label{thm:thermodynamic-immunity}
For every bath temperature \(T<M_N\), the re-excitation rate
\(\Gamma_{\bar B\to B}(T)\) vanishes identically, so the decoupled
anti-baryon charge sector is a decoherence-free sector of the Lindblad
evolution~\eqref{eq:lindblad-master-equation}: no environment mode at
\(T<M_N\) carries the energy required to reactivate a \(\bar B\to B\)
transition, and the encoded neutral state \(\rho_\star\) is a fixed point of
the channel.
\end{theorem}

\begin{proof}
Each jump operator~\eqref{eq:lindblad-jump-operators} transfers charge only
if the environment can supply the seed-dissolution energy \(M_N\).  At
\(T<M_N\) the occupation numbers of such modes vanish identically in the
model, so every \(\gamma_k(T)\) entering a \(\bar B\to B\) jump is zero and
the dissipator restricted to that channel collapses to zero.  The remaining
Lindblad terms act within the neutral subspace or on the environment alone,
hence \(\mathcal L\rho_\star=0\).  The native check
\pyvar{BaryogenesisOptimizer::thermal\_lindblad\_evolution}$(T,M_N)$
evaluates Eq.~\eqref{eq:thermal-immunity-rate} and returns exactly \(0\)
for all \(T<M_N\) (and \(e^{-1}\) at the threshold \(T=M_N\)).
\end{proof}

Above the threshold the same rate controls the redshift-resolved loading
history.  Writing \(f_H(z)=(H_\Lambda/H_{\mathrm{eff}}(z))^3\) for the
horizon-conditioning factor, the transported baryon asymmetry obeys the
continuous differential transport equation
\begin{equation}
\label{eq:eta-b-transport}
 \frac{d\eta_B}{d\log_{10}z}
 =
 -\left(\eta_B^\star-\eta_B\right)R(\log_{10}z),
 \qquad
 R(u)=A\,b\,\ln10\;10^{-bu},
\end{equation}
with the boundary condition \(\eta_B(z=10^{16})=0\) at the \(SO(10)\)
restoration scale and the attractor
\(\eta_B^\star=6.449923359416\times10^{-10}\).  The equation is integrated
by \pyvar{CausalPoint::thermal\_history\_trajectory}$(z_{\mathrm{start}},
z_{\mathrm{end}})$ by fourth-order Runge--Kutta over
\(z\in[10^{9},10^{16}]\), and the trajectory converges to the fixed point
at the electroweak-to-BBN transition, summarized in
Table~\ref{tab:eta-b-thermal-history}.
...
\(z\) & \(\eta_B(z)\) & Phase \\
\midrule
\(10^{16}\) & \(0\) & \(SO(10)\) restoration \\
\(10^{14}\) & \(1.241052\times10^{-10}\) & GUT era \\
\(10^{12}\) & \(5.892110\times10^{-10}\) & electroweak era \\
\(10^{9}\) & \(6.449923\times10^{-10}\) & BBN fixed point \\
\(10^{4}\) & \(6.449923\times10^{-10}\) & fixed point \\
\(1.1\times10^{3}\) & \(6.449923\times10^{-10}\) & equality/recombination \\
\bottomrule
\end{tabular}
\caption{Horizon-conditioned baryon-asymmetry trajectory
\(\eta_B(z)\) from Eq.~\eqref{eq:eta-b-transport}: the integrated history
runs from vanishing asymmetry at the \(SO(10)\) restoration scale through
the GUT and electroweak epochs into the attractor
\(\eta_B^\star=6.449923359416\times10^{-10}\).}
\label{tab:eta-b-thermal-history}
\end{table}
...
$\ln2$ converting the area entropy into bits.  These relations tie the global
screen capacity \(\Nsat=3\pi/(\Lp^2\Lholo)\) directly to the local observer:
the same saturated register that fixes the cosmological budget bounds the
memory \(N_{\text{limit}}\) available to any single Causal Point, so observer
mechanics inherits its limits from the global holographic ledger rather than
from an independent postulate.
...
The native transcript \pyvar{codespace_audit.json} supplies a foundation
summary on \((26,8,312)\), with zero reported framing defect, true modular
and zero-energy flags, nine metric slices, nine history entries, and a true
stress-energy-preservation flag.  Its reported baryon asymmetry is
\(6.449923359416131\times10^{-10}\).  These are native execution results
reported in the supplied artifact, not an independent source-level audit;
Section~\ref{sec:reported-run-provenance} documents their scope.
...
The numerical tables in this section are reconciled with the precision
report embedded in \pyvar{codespace_audit.json}.  The earlier
\pyvar{simulator_output.txt} is retained as a historical comparison.
Section~\ref{sec:reported-run-provenance}
records its execution scope and exact dictionary-field crosswalk.  Agreement
with that report is an output-consistency check, not an independent proof of
the microscopic construction or a new observational detection.

Exact rational ledger identities are distinguished from retrodictive
comparisons of model outputs with established quantities, including the
baryon asymmetry, dark-matter abundance, neutrino scale, and horizon register.
The designation ``zero-parameter structural retrodiction'' requires a
separate demonstration that every input is fixed by the branch or by
specified fundamental constants.  The supplied report alone does not provide
that demonstration: it contains an explicit CMB intercept and comparison
parameters, and the neutrino hierarchy uses oscillation-splitting inputs.
The growth suppression and CPL template remain observational predictions;
the address-dependent heat slope and capacity cutoff remain proposed
laboratory tests.  The calorimetric slope is \(k_BT\ln2\) per operation,
or \(N_{\text{pulse}}k_BT\ln2\simeq6.6990\,\mathrm{zJ/bit}\) for the
specified \(100{,}000\)-operation burst at \(7\,\mathrm{mK}\).

\subsection{First-Principles Cosmic Inflation from Boundary Loading}
\label{sec:first-principles-shbt-inflation}

The canonical branch replaces the scalar-field pair $(\phi,V)$ by the finite
boundary observable
\begin{equation}
\label{eq:inflation-loading-fraction}
 \fload(z)
 =
 \frac{N_{\mathrm{load}}(z)}{\Nsat},
 \qquad
 ...
 =3.311997720142366\times10^{122}\ \mathrm{bits}.
\end{equation}
The exact bit-loading equation and its canonical normalization are
\begin{align}
\label{eq:inflation-loading-ode}
 \frac{d\fload}{dz}
 &=
 \frac{\GammaLock}{(1+z)^2H_{\mathrm{SHBT}}(z)},
 \\
\label{eq:inflation-lock-normalization}
 \GammaLock
 &=3\AH
 =3\hcmb\left[\exp\!\left(\frac{\DeltaMod}{2}\right)-1\right]
 =14.393880218584\ \kmsmpc,
...
\end{align}
The sign of Eq.~\eqref{eq:inflation-loading-ode} is defined with respect to the
past-light-cone acquisition orientation.  If $z_{\mathrm{sp}}$ denotes the
usual spectroscopic redshift increasing into the past, then
\begin{equation}
\label{eq:lightcone-redshift-orientation}
 dz=-dz_{\mathrm{sp}},
 \qquad
 \frac{d\fload}{dz_{\mathrm{sp}}}
 =-\frac{\GammaLock}
 {(1+z_{\mathrm{sp}})^2H_{\mathrm{SHBT}}(z_{\mathrm{sp}})}.
\end{equation}
This orientation makes the analytic high-redshift solution and the finite
register boundary condition mutually consistent.

The expansion scalar is fixed by the perception identity
\begin{equation}
\label{eq:inflation-perception-identity}
 \boxed{
 \dot{\mathcal T}
 =
 \frac{1}{\Nsat}\frac{dS_{\mathrm{index}}}{dt}
 =H(t).}
\end{equation}
The observer-frame scale factor therefore parametrizes the rate of indexed
boundary information.  There is no potential-dominated stress tensor, no
slow-roll trajectory, and no independent reheating field; thermal radiation
and boundary loading are distinct channels whose coupling vanishes rapidly at
large redshift.

In the radiation regime,
\begin{equation}
\label{eq:inflation-radiation-hubble}
 H_{\mathrm{rad}}(z)
 =
 ...
 =0.646478\ \kmsmpc.
\end{equation}
Equations~\eqref{eq:inflation-loading-ode} and
\eqref{eq:inflation-radiation-hubble}, expressed in spectroscopic orientation,
give
\begin{equation}
\label{eq:inflation-radiation-loading-derivative}
 -\frac{d\fload}{dz}
 =
 \frac{\GammaLock}
 {\hcmb\sqrt{\Omega_{r0}}(1+z)^4}.
\end{equation}
The boundary condition $\fload(z\rightarrow\infty)=0$ yields
\begin{align}
\label{eq:inflation-radiation-asymptotic-solution}
 \fload^{\mathrm{rad}}(z)
 &=
 \int_z^\infty
 \frac{\GammaLock\,dz'}
 ...
 =
 \frac{\GammaLock}
 {3\hcmb\sqrt{\Omega_{r0}}(1+z)^3}
 \notag\\
 &=
 \frac{\AH}
 {\hcmb\sqrt{\Omega_{r0}}(1+z)^3},
 \\
\label{eq:inflation-radiation-entropy-debt}
 \debtentropy^{\mathrm{rad}}(z)
 &=
 \Nsat\fload^{\mathrm{rad}}(z)
 =
 \frac{\Nsat\AH}
 {\hcmb\sqrt{\Omega_{r0}}(1+z)^3}.
\end{align}
The clock-skew perturbation is
\begin{equation}
\label{eq:inflation-clock-skew}
 \delta H_{\mathrm{load}}(z)
 =
 \frac{\AH}{1+z}.
\end{equation}
At the BBN benchmark $z_{\mathrm{BBN}}=10^9$, the complete numerical ledger is
\begin{align}
\label{eq:inflation-bbn-decoupling-proof}
 \delta H_{\mathrm{load}}(10^9)
 &=4.797960068064\times10^{-9}\ \kmsmpc,
 \\
 H_{\mathrm{rad}}(10^9)
 &=6.46478\times10^{17}\ \kmsmpc,
 \\
 \frac{\delta H_{\mathrm{load}}(10^9)}
 {H_{\mathrm{rad}}(10^9)}
 &=
 \frac{\AH}{\hcmb\sqrt{\Omega_{r0}}}\times10^{-27}
 \notag\\
 &=
 \frac{4.797960072861}{0.646478}\times10^{-27}
 =7.42169\times10^{-27}.
\end{align}
Thus the active boundary channel is smaller than the radiation expansion rate
by more than twenty-six orders of magnitude.  Standard nuclear reaction
networks evolve on the unmodified radiation background to this precision, so
the SHBT construction requires no scalar decay or reheating stage to recover
the BBN thermal regime.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabular}{@{}L{0.15\linewidth}rrrrr@{}}
\toprule
Epoch and redshift & $H_{\mathrm{bg}}$ & $\delta H_{\mathrm{load}}$ & $\delta H/H_{\mathrm{bg}}$ & $\fload$ & $\debtentropy$ [bits] \\
 & \multicolumn{2}{c}{$[\kmsmpc]$} & & & \\
\midrule
BBN peak, $10^9$ & $6.46478\times10^{17}$ & $4.79796\times10^{-9}$ & $7.42169\times10^{-27}$ & $7.42169\times10^{-27}$ & $2.45806\times10^{96}$ \\
Post-BBN, $10^7$ & $6.46478\times10^{13}$ & $4.79796\times10^{-7}$ & $7.42169\times10^{-21}$ & $7.42169\times10^{-21}$ & $2.45806\times10^{102}$ \\
Early radiation, $10^5$ & $6.46478\times10^{9}$ & $4.79796\times10^{-5}$ & $7.42169\times10^{-15}$ & $7.42169\times10^{-15}$ & $2.45806\times10^{108}$ \\
Matter--radiation equality, $3.4\times10^3$ & $7.47300\times10^{6}$ & $1.41075\times10^{-3}$ & $1.88780\times10^{-10}$ & $1.88780\times10^{-10}$ & $6.25239\times10^{112}$ \\
Recombination, $1.1\times10^3$ & $7.82238\times10^{5}$ & $4.35782\times10^{-3}$ & $5.57096\times10^{-9}$ & $5.57096\times10^{-9}$ & $1.84510\times10^{114}$ \\
Cosmic noon, $2$ & $2.03500\times10^{2}$ & $1.59932$ & $7.85907\times10^{-3}$ & $2.74877\times10^{-1}$ & $9.10393\times10^{121}$ \\
Present, $0$ & $6.73600\times10^{1}$ & $4.79796$ & $7.12286\times10^{-2}$ & $1.00000$ & $3.311998\times10^{122}$ \\
\bottomrule
\end{tabular}
\caption{Early-universe bit-loading and BBN-decoupling ledger.  The first
three rows use the radiation asymptote; the remaining rows use the full
background expansion and the present-normalized finite-register solution.
Hubble quantities are in $\kmsmpc$, and displayed precision is numerical
rather than observational.}
\label{tab:early-universe-loading-ledger}
\end{table}

The canonical single-clock perturbation sector supplies the tensor and
collapsed-limit non-Gaussianity invariants
\begin{align}
\label{eq:inflation-tensor-invariants}
 r
 &\equiv
 \frac{P_t(k_*)}{P_{\mathcal R}(k_*)}
 =0.0032,
 &
 n_t&=-\frac{r}{8}=-0.0004,
 \\
\label{eq:inflation-suyama-johnston}
 \tau_{\mathrm{NL}}
 &=
 \left(\frac{6}{5}f_{\mathrm{NL}}^{\mathrm{local}}\right)^2
 =\frac{36}{25}(0.015)^2
 =0.000324.
\end{align}
Equation~\eqref{eq:inflation-suyama-johnston} is the saturated
Suyama--Johnston relation: independent entropic directions are absent because
all physical perturbations project onto the single boundary loading clock.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}L{0.24\linewidth}L{0.29\linewidth}L{0.19\linewidth}Y@{}}
\toprule
Domain & CSV or JSON crosswalk & Canonical value & Mathematical field \\
\midrule
Tensor CMB spectrum & \pyvar{shbt_run_tensor_cls.csv}; \pyvar{precision_pipeline.spectra.r} & $r=0.0032$, $n_t=-0.0004$ & Eq.~\eqref{eq:inflation-tensor-invariants} \\
Scalar CMB spectrum & \pyvar{shbt_run_cmb_cls.csv}; \pyvar{precision_pipeline.spectra.f_NL_local} & $f_{\mathrm{NL}}^{\mathrm{local}}=0.015$ & Eq.~\eqref{eq:inflation-suyama-johnston} \\
Trispectrum & \pyvar{precision_pipeline.non_gaussianity.tau_NL} & $\tau_{\mathrm{NL}}=0.000324$ & single-clock equality \\
Matter spectrum & \pyvar{shbt_run_matter_pk.csv} & branch-normalized $P_m(k,z)$ & loaded growth equation \\
Loading ledger & \pyvar{lightcone_entropy_debt}; \pyvar{bbn_stability} & $7.42169\times10^{-27}$ at BBN & Eq.~\eqref{eq:inflation-bbn-decoupling-proof} \\
Calorimetry & \pyvar{shbt_run_calorimetry_sim.csv} & $T=7\,\mathrm{mK}$; slope $k_BT\ln2$ & address-scaled Landauer channel \\
\bottomrule
\end{tabularx}
\caption{Cross-domain numerical crosswalk between analytic inflationary
invariants and machine-readable simulation outputs.}
\label{tab:inflation-data-crosswalk}
\end{table}
...
\label{eq:cosmology-dark-ledger}
 \begin{aligned}
 \cdarkres&\equiv\frac{834433}{362670}
 \simeq2.300805139659,\\
 \cdarkcomp&\equiv\cdarkres+1=\frac{1197103}{362670}
 \simeq3.300805139659,\\
 \DeltaMod&\equiv\frac{\cdarkcomp}{24}
 =\frac{1197103}{8704080}\simeq0.137533547486.
 \end{aligned}
\end{equation}
...
Eq.~\eqref{eq:dark-completion-fraction}, verifies the unit difference between
the two ledgers, while Appendix~\ref{app:center-directed-modular-framing}
...
\label{eq:cosmology-finite-register}
 ...
 \qquad
 \Nsat\simeq3.311997720142366\times10^{122}.
\end{equation}
...
The native value supersedes the earlier Python-only fallback
\(3.312593327986\times10^{122}\), a change of approximately
\(-0.0179801\%\).  Table~\ref{tab:lightcone-entropy-debt} uses the native
value throughout.  Its effect on the neutrino normalization is reconciled by
the sub-leading boundary-cell weight in
Eq.~\eqref{eq:native-neutrino-normalization-check}.
...
stationarity check gives \(\dot G/G=0\).  The same branch is intended to
satisfy the unity-of-scale matching relations.  The retained \(D_5\) cell factor \(\kappad\) entering
...
\label{eq:cosmology-unity-scale}
 m_{\nu,1}=\kappad^\star M_P N^{-1/4},
 \qquad
 \Lholo=\frac{3\pi}{(\kappad^\star)^4}G_{\text{top}}m_{\nu,1}^4,
 \qquad
...
Physical operator or observable & Mathematical definition & Theoretical value & Closure condition \\
\midrule
Completed ledger & $\cdarkcomp=\cdarkres+1$ & $1197103/362670\simeq3.300805139659$ & exact rational closure \\
Entropy debt & $\DeltaMod=\cdarkcomp/24$ & $0.137533547486$ & fixed by ledger \\
$D_5$ cell measure & $\kappad^\star$ & $0.988725345719$ & sub-leading finite-screen closure \\
Holographic curvature & $\Lholo$ & $1.08913883\times10^{-52}\ \mathrm{m}^{-2}$ & fixed screen \\
Horizon register & $N(\Lholo)=3\pi/(\Lp^2\Lholo)$ & $3.311997720142366\times10^{122}$ & native register output \\
CMB intercept & $H_0^{\text{CMB}}$ & $67.4\,\kmsmpc$ & early-time anchor \\
Neutrino floor & $m_{\nu,1}=\kappad^\star M_P\Nsat^{-1/4}$ & $2.829630635353\times10^{-3}\ \mathrm{eV}$ & exact register/output unification \\
Normal mass sum & $\Sigma_{\nu}^{\text{NH}}$ & $61.965396\ \mathrm{meV}$ & reported normal-ordering output \\
Inverted mass sum & $\Sigma_{\nu}^{\text{IH}}$ & $103.723086\ \mathrm{meV}$ & reported comparison output \\
\bottomrule
\end{tabular}
\caption{Completed-ledger observables and Noether-bridge matching conditions.
The sub-leading boundary-cell correction in
Eq.~\eqref{eq:native-neutrino-normalization-check} reconciles the native
finite register with the reported neutrino floor.}
\label{tab:noether-bridge-audit}
\end{table}

\subsection{Sub-Leading Boundary-Cell Weight Corrections}
\label{sec:subleading-boundary-cell-weights}

At leading order, the finite-register matching relation assigns the neutrino
floor to the quarter-root screen scale weighted by the retained \(D_5\)
boundary-cell measure:
\begin{equation}
...
\end{equation}
Substituting the native register value and the stated constants gives
\begin{align}
\label{eq:neutrino-floor-numeric-substitution}
 ...
 \left(3.311997720142366\times10^{122}\right)^{-1/4}
 \notag\\
 &\simeq
 2.829757842307\times10^{-3}\ \mathrm{eV}.
\end{align}
The difference from the reported output floor
\begin{equation}
\label{eq:neutrino-output-floor}
 m_{\nu,1}^{\text{out}}
 =
 2.829630635353\times10^{-3}\ \mathrm{eV}
\end{equation}
is the first finite-screen Cardy--Verlinde correction to the retained
boundary-cell weight.  Let the legacy register normalization be denoted by
\(\Nsat^{\text{legacy}}\).  The sub-leading correction is
\begin{align}
\label{eq:subleading-d5-correction}
 \Delta_{D_5}^{\text{sub}}
 &\equiv
 1-
 \left(
 \frac{\Nsat}{\Nsat^{\text{legacy}}}
 \right)^{1/4}
 \notag\\
 &=
 1-
 \left(
 \frac{3.311997720142366\times10^{122}}
 {3.312593326237\times10^{122}}
 \right)^{1/4}
 =
 4.4953168\times10^{-5}.
\end{align}
The renormalized \(D_5\) boundary measure is consequently
\begin{align}
\label{eq:renormalized-d5-measure}
 \kappad^\star
 &=
 \kappad\left(1-\Delta_{D_5}^{\text{sub}}\right)
 =
 0.988725345719,
 \\
\label{eq:native-neutrino-normalization-check}
 m_{\nu,1}
 &=
 \kappad^\star M_P\Nsat^{-1/4}
 =
 2.829630635353\times10^{-3}\ \mathrm{eV}
 =
 m_{\nu,1}^{\text{out}}.
\end{align}
Thus the leading \(0.0045\%\) mismatch is absorbed by the unique
quarter-root finite-screen correction, and the native register formula and
output floor coincide at the displayed precision without changing \(M_P\),
\(\Lholo\), or the completed dark ledger.

The oscillation-splitting inputs used for that comparison are
\begin{equation}
\label{eq:neutrino-oscillation-splittings}
 \Delta m_{21}^2=7.4\times10^{-5}\ \mathrm{eV}^2,
 \qquad
 |\Delta m_{31}^2|=2.5\times10^{-3}\ \mathrm{eV}^2.
\end{equation}
...
\label{eq:normal-hierarchy-eigenstates}
 ...
 &=
 \sqrt{m_{\nu,1}^2+\Delta m_{21}^2}
 \simeq9.055761124\times10^{-3}\ \mathrm{eV},\\
 m_{\nu,3}^{\text{NH}}
 &=
 \sqrt{m_{\nu,1}^2+\Delta m_{31}^2}
 \simeq5.008000409\times10^{-2}\ \mathrm{eV}.\notag
\end{align}
...
\label{eq:normal-hierarchy-sum}
 ...
 +m_{\nu,3}^{\text{NH}}
 \simeq
 61.965396\ \mathrm{meV}
 \simeq61.97\ \mathrm{meV}.
\end{equation}
For the inverted hierarchy, the finite-register floor is assigned to the
lightest third eigenstate.  The comparison uses the same atmospheric-gap
magnitude in Eq.~\eqref{eq:neutrino-oscillation-splittings}:
\begin{align}
\label{eq:inverted-hierarchy-eigenstates}
 m_{\nu,3}^{\text{IH}}
 &=2.829630635353\times10^{-3}\ \mathrm{eV},\notag\\
 m_{\nu,1}^{\text{IH}}
 &=\sqrt{(m_{\nu,3}^{\text{IH}})^2+|\Delta m_{31}^2|}
 \simeq5.008000409\times10^{-2}\ \mathrm{eV},\\
 m_{\nu,2}^{\text{IH}}
 &=\sqrt{(m_{\nu,3}^{\text{IH}})^2+|\Delta m_{31}^2|+\Delta m_{21}^2}
 \simeq5.081345107\times10^{-2}\ \mathrm{eV}.\notag
\end{align}
so that
\begin{equation}
\label{eq:inverted-hierarchy-sum}
 ...
 \sqrt{(m_{\nu,3}^{\text{IH}})^2+|\Delta m_{31}^2|+\Delta m_{21}^2}
 \simeq
 103.723086\ \mathrm{meV}
 \simeq103.72\ \mathrm{meV}.
\end{equation}
A hierarchy statistic would compare each derived sum with a specified
...
\label{eq:neutrino-tension-sigma}
 ...
 {\sigma_{\Sigma}^{\text{IH}}}.
\end{equation}
The supplied output contains neither this likelihood nor updated pull values.
Consequently no numerical hierarchy preference is inferred from this run.
Table~\ref{tab:neutrino-hierarchy-audit} uses the computed eigenvalue arrays and
sums; the separate legacy display fields in the report are not used as
numerical results.

\begin{table}[htbp]
\centering
\small
\begin{tabularx}{\linewidth}{@{}L{0.21\linewidth}Yrr@{}}
\toprule
Physical observable & Eigenstate or definition & Normal (meV) & Inverted (meV) \\
\midrule
Floor eigenstate & \(m_1^{\text{NH}}\), \(m_3^{\text{IH}}\) & 2.829631 & 2.829631 \\
Middle eigenstate & \(m_2^{\text{NH}}\), \(m_1^{\text{IH}}\) & 9.055761 & 50.080004 \\
Heaviest eigenstate & \(m_3^{\text{NH}}\), \(m_2^{\text{IH}}\) & 50.080004 & 50.813451 \\
Mass sum & \(\Sigma_\nu=\sum_i m_{\nu,i}\) & 61.965396 & 103.723086 \\
\bottomrule
\end{tabularx}
\caption{Neutrino benchmark outputs from the supplied
\pyvar{neutrino_hierarchy} record, converted from eV to meV and rounded after
evaluation.  These conditional mass predictions use the stated oscillation
splittings; the run does not evaluate hierarchy likelihood pulls.}
\label{tab:neutrino-hierarchy-audit}
\end{table}

The hierarchy check is therefore tied to the same
\(\kappad^\star\), \(\Nsat\), and completed-ledger inputs as the Newton lock.
...
$D_5$ measure & $\kappad^\star=\kappad(1-\Delta_{D_5}^{\text{sub}})$ & renormalized boundary-cell weight & finite-screen closure \\
Horizon register & $\Nsat=3\pi/(\Lp^2\Lholo)$ & finite information capacity & saturated screen \\
Neutrino floor & $m_{\nu,1}=\kappad^\star M_P\Nsat^{-1/4}$ & absolute mass scale & unity of scale \\
...
Boltzmann hierarchy & $\mathbf y'=\mathbf F(\eta,k,q,\mathbf y)$ & first-order CMB and matter transport & Einstein--Boltzmann closure \\
Tensor spectrum & $r,n_t,C_\ell^{BB}$ & primordial gravitational waves and B modes & canonical tensor closure \\
Non-Gaussianity & $f_{\mathrm{NL}}^{X},\tau_{\mathrm{NL}},g_{\mathrm{NL}}$ & bispectrum and trispectrum response & single-clock saturation \\
...
the first-order Einstein--Boltzmann system is
Eqs.~\eqref{eq:shbt-newtonian-gauge-metric}--\eqref{eq:shbt-boltzmann-state-vector};
the tensor and higher-order statistics are
Eqs.~\eqref{eq:shbt-tensor-metric}--\eqref{eq:shbt-suyama-johnston-proof}
and Tables~\ref{tab:shbt-tensor-expansion-phases}--\ref{tab:shbt-non-gaussianity-parameters};
...
\midrule
0.0 & 4.797960 & 72.197960 \\
0.5 & 3.198640 & 70.598640 \\
1.0 & 2.398980 & 69.798980 \\
2.0 & 1.599320 & 68.999320 \\
10.0 & 0.436178 & 67.836178 \\
1100.0 & 0.004358 & 67.404358 \\
\bottomrule
\end{tabular}
\caption{Conditional observer-frame Hubble-loading predictions, rounded from
the supplied \pyvar{redshift_ladder} record.  The run uses
\(\hcmb=67.4\,\kmsmpc\); the displayed digits indicate numerical rounding,
not observational precision.}
\label{tab:precision-redshift-ladder}
\end{table}

\subsection{First-Order Holographic Cosmological Perturbation Theory and Boltzmann Pipeline}
\label{sec:first-order-shbt-boltzmann}

Scalar perturbations are evolved in conformal Newtonian gauge,
\begin{equation}
\label{eq:shbt-newtonian-gauge-metric}
 ds^2
 =
 a^2(\eta)
 \left[
 ...
 \right].
\end{equation}
The boundary clock-skew loading enters only through the background conformal
Hubble rate,
\begin{align}
\label{eq:shbt-conformal-hubble}
 \mathcal H(a)
 &\equiv
 \frac{a'}{a}
 =
 aH_{\text{SHBT}}(a)
 =
 a\left(\hcmb+\AH a\right)E(a),
 \\
\label{eq:shbt-background-ea}
 E(a)
 &=
 \left[
 \Omega_{r0}a^{-4}
 ...
 H_0(a)=\hcmb+\AH a.
\end{align}
Since \(d/d\eta=\mathcal H\,d/d\ln a\), differentiation gives
\begin{align}
\label{eq:shbt-conformal-acceleration}
 \mathcal H'
 &=
 \mathcal H^2
 \left[
 1+
 \left(\frac{d\ln E}{d\ln a}\right)_{\lcdm}
 +
...
For a Fourier mode of comoving wave number \(k\), the linearized Einstein
constraints are
\begin{align}
\label{eq:shbt-einstein-energy-constraint}
 k^2\Phi
 +3\mathcal H\left(\Phi'+\mathcal H\Psi\right)
 &=
 4\pi G_{\text{eff}}a^2\delta\rho,
 \\
\label{eq:shbt-einstein-momentum-constraint}
 k^2\left(\Phi'+\mathcal H\Psi\right)
 &=
 -4\pi G_{\text{eff}}a^2
 \sum_s(\rho_s+p_s)\theta_s,
 \\
\label{eq:shbt-einstein-shear-constraint}
 k^2(\Phi-\Psi)
 &=
 12\pi G_{\text{eff}}a^2
 \sum_s(\rho_s+p_s)\sigma_s.
\end{align}
The Newton lock keeps \(G_{\text{eff}}\) constant; boundary loading modifies
the perturbation system through \(\mathcal H\), \(\mathcal H'\), and the
potential drift rather than through a time-dependent gravitational coupling.

The pressureless dark-matter hierarchy is
\begin{align}
\label{eq:shbt-dark-matter-perturbations}
 \delta_{\text{DM}}'
 &=
 -\theta_{\text{DM}}+3\Phi',
 \\
 \theta_{\text{DM}}'
 &=
 -\mathcal H\theta_{\text{DM}}+k^2\Psi.
\end{align}
For baryons, with sound speed \(c_b\), Thomson opacity
\(\dot\kappa=a n_e\sigma_T\), photon velocity
\(\theta_\gamma=3k\Theta_1\), and
\(R_b=3\rho_b/(4\rho_\gamma)\),
\begin{align}
\label{eq:shbt-baryon-perturbations}
 \delta_b'
 &=
 -\theta_b+3\Phi',
 \\
 ...
 +k^2\Psi
 +\frac{\dot\kappa}{R_b}
 \left(3k\Theta_1-\theta_b\right).
\end{align}
The photon temperature multipoles obey
\begin{align}
\label{eq:shbt-photon-monopole}
 \Theta_0'
 &=
 -k\Theta_1-\Phi',
 \\
\label{eq:shbt-photon-dipole}
 \Theta_1'
 &=
 \frac{k}{3}
 \left(\Theta_0+\Psi-2\Theta_2\right)
 -\dot\kappa
 \left(
 \Theta_1-\frac{\theta_b}{3k}
 \right),
 \\
\label{eq:shbt-photon-quadrupole}
 \Theta_2'
 &=
 \frac{k}{5}
 \left(2\Theta_1-3\Theta_3\right)
 -\dot\kappa
 \left(
 \Theta_2-\frac{\Pi_\gamma}{10}
 \right),
 \\
\label{eq:shbt-photon-hierarchy}
 \Theta_\ell'
 &=
 \frac{k}{2\ell+1}
 \left[
...
For massive neutrinos, let \(q\) be the comoving momentum,
\(\epsilon(q,a)=\sqrt{q^2+a^2m_\nu^2}\), and \(f_{\nu0}(q)\) the unperturbed
Fermi--Dirac distribution.  The collisionless Boltzmann hierarchy is
\begin{align}
\label{eq:shbt-neutrino-monopole}
 \mathcal N_0'
 &=
 -\frac{qk}{\epsilon}\mathcal N_1
 +\Phi'
 \frac{\partial\ln f_{\nu0}}{\partial\ln q},
 \\
\label{eq:shbt-neutrino-dipole}
 \mathcal N_1'
 &=
 \frac{qk}{3\epsilon}
 \left(\mathcal N_0-2\mathcal N_2\right)
 -\frac{\epsilon k}{3q}\Psi
 \frac{\partial\ln f_{\nu0}}{\partial\ln q},
 \\
\label{eq:shbt-neutrino-hierarchy}
 \mathcal N_\ell'
 &=
 \frac{qk}{(2\ell+1)\epsilon}
 \left[
 \ell\mathcal N_{\ell-1}
 -(\ell+1)\mathcal N_{\ell+1}
 \right],
 \qquad
 \ell\ge2.
\end{align}
The mass parameter in \(\epsilon(q,a)\) is supplied by the reconciled
finite-screen floor of Eq.~\eqref{eq:native-neutrino-normalization-check} and
the hierarchy eigenvalues in
Eqs.~\eqref{eq:normal-hierarchy-eigenstates} and
\eqref{eq:inverted-hierarchy-eigenstates}.

Let \(D(a)\) denote the linear matter growth factor and
\begin{equation}
\label{eq:shbt-growth-rate-definition}
 f(a)
 \equiv
 \frac{d\ln D}{d\ln a}.
\end{equation}
On subhorizon scales with negligible total anisotropic stress,
\(\Psi=\Phi\), while the loaded potential scales as
\(\Phi\propto D/[aH_0(a)]\).  Therefore
\begin{align}
\label{eq:shbt-potential-drift-derivation}
 \frac{\Phi'}{\mathcal H\Phi}
 &=
 \frac{d\ln\Phi}{d\ln a}
 =
 f(a)-1-\frac{d\ln H_0}{d\ln a},
 \\
\label{eq:shbt-potential-drift}
 \Phi'+\Psi'
 &=
 2\mathcal H\Phi
 \left[
 ...
 \right].
\end{align}
The first bracket is the standard growth contribution.  The pure
boundary-clock contribution is the exact logarithmic ISW source
\begin{equation}
\label{eq:shbt-boltzmann-isw-source}
 \boxed{
 \Delta_{\text{ISW}}(z)
 =
 ...
 -\frac{2\AH}{(1+z)H_0(z)}.}
\end{equation}
The corresponding line-of-sight transfer is
\begin{align}
\label{eq:shbt-isw-line-of-sight}
 S_{\text{ISW}}(k,\eta)
 &=
 e^{-\kappa(\eta)}
 \left(\Phi'+\Psi'\right),
 \\
 \Theta_\ell^{\text{ISW}}(k,\eta_0)
 &=
 \int_0^{\eta_0}
 d\eta\,
 S_{\text{ISW}}(k,\eta)
 j_\ell\!\left[k(\eta_0-\eta)\right].
\end{align}

The Python implementation contract for \pyvar{boltzmann_shbt.py} is organized
around the state vector
\begin{equation}
\label{eq:shbt-boltzmann-state-vector}
 \mathbf y(k,q,\eta)
 =
 \left(
 \Phi,\Psi,
 \delta_{\text{DM}},\theta_{\text{DM}},
 \delta_b,\theta_b,
 \Theta_0,\ldots,\Theta_{\ell_\gamma},
 \mathcal N_0,\ldots,\mathcal N_{\ell_\nu}
 \right)^T,
 \qquad
 \mathbf y'=\mathbf F(\eta,k,q,\mathbf y).
\end{equation}
\begin{table}[htbp]
\centering
\small
...
\toprule
Python interface & Required operation \\
\midrule
\pyvar{ShbtBackground} & Evaluate \(H_0(a)\), \(E(a)\), \(\mathcal H(a)\), \(\mathcal H'(a)\), optical depth, and recombination inputs. \\
\pyvar{initial_adiabatic_state} & Construct superhorizon adiabatic initial data for all fluid and multipole variables. \\
\pyvar{einstein_constraints} & Solve Eqs.~\eqref{eq:shbt-einstein-energy-constraint}--\eqref{eq:shbt-einstein-shear-constraint} for \(\Phi\) and \(\Psi\). \\
\pyvar{boltzmann_rhs} & Assemble Eqs.~\eqref{eq:shbt-dark-matter-perturbations}--\eqref{eq:shbt-neutrino-hierarchy} on the \((k,q,\eta)\) grid. \\
\pyvar{integrate_mode} & Integrate the stiff pre-recombination system and switch to the free-streaming hierarchy after decoupling. \\
\pyvar{line_of_sight_sources} & Construct visibility, Sachs--Wolfe, Doppler, polarization, and ISW source functions. \\
\pyvar{transfer_functions} & Return \(T_\delta(k,z)\), \(T_\Phi(k,z)\), \(T_\Psi(k,z)\), and \(\Theta_\ell(k)\). \\
\pyvar{tensor_mode_rhs} / \pyvar{tensor_bmode_transfer} & Integrate Eq.~\eqref{eq:shbt-tensor-mode-ode} and construct \(\Delta_{T,\ell}^{(t)}(k)\) and \(C_\ell^{BB}\). \\
\pyvar{non_gaussianity_templates} & Evaluate \(S_{\text{equil}}\), \(S_{\text{ortho}}\), \(f_{\mathrm{NL}}^X\), \(\tau_{\mathrm{NL}}\), and \(g_{\mathrm{NL}}\). \\
\pyvar{write_boltzmann_outputs} & Export background, perturbation, CMB, matter, and neutrino transfer tables to CSV and JSON. \\
\bottomrule
\end{tabularx}
\caption{Required first-order SHBT Boltzmann interfaces and their mathematical
responsibilities in \pyvar{boltzmann_shbt.py}.}
\label{tab:shbt-boltzmann-pipeline}
\end{table}

\subsection{Primordial Tensor Modes and CMB B-Mode Power Spectra}
\label{sec:primordial-tensor-b-modes}

The tensor sector is defined by the transverse-traceless spatial perturbation
\begin{equation}
\label{eq:shbt-tensor-metric}
 ds^2
 =
 a^2(\eta)
 \left[
 -d\eta^2+
 \left(\delta_{ij}+h_{ij}\right)dx^i dx^j
 \right],
 \qquad
 \partial_i h_{ij}=0,
 \qquad
 h_i{}^i=0.
\end{equation}
For each helicity mode and comoving wave number \(k\), Newton-lock
stationarity leaves the propagation equation unmodified except through the
loaded conformal background:
\begin{equation}
\label{eq:shbt-tensor-mode-ode}
 \boxed{
 h_{ij}''+2\mathcal H h_{ij}'+k^2h_{ij}=0,
 \qquad
 \mathcal H(a)
 =
 a\left(\hcmb+\AH a\right)E(a).}
\end{equation}
Writing the dimensionless primordial tensor spectrum at pivot scale \(k_*\) as
\begin{equation}
\label{eq:shbt-primordial-tensor-spectrum}
 \mathcal P_t(k)
 =
 rA_s
...
\end{equation}
the canonical branch fixes
\begin{equation}
\label{eq:shbt-canonical-tensor-values}
 (\kell,\kq,\Kpar)=(26,8,312),
 \qquad
 r=0.0032,
 \qquad
...
Let \(\Delta_{T,\ell}^{(t)}(k)\) denote the tensor-polarization transfer
function obtained by integrating Eq.~\eqref{eq:shbt-tensor-mode-ode} through
recombination and reionization.  Its line-of-sight representation is
\begin{equation}
\label{eq:shbt-tensor-transfer-function}
 \Delta_{T,\ell}^{(t)}(k)
 =
 \int_0^{\eta_0}
 d\eta\,
 S_B^{(t)}(k,\eta)
 \beta_\ell\!\left[k(\eta_0-\eta)\right],
\end{equation}
where \(S_B^{(t)}\) is the tensor polarization source and
\(\beta_\ell\) is the spin-two radial projection kernel.  The primordial
CMB B-mode angular power spectrum is therefore
\begin{equation}
\label{eq:shbt-cmb-bb-spectrum}
 \boxed{
 C_\ell^{BB}
 =
 (4\pi)^2
 ...
 \right|^2.}
\end{equation}
Equations~\eqref{eq:shbt-tensor-mode-ode}--\eqref{eq:shbt-cmb-bb-spectrum}
separate the branch-fixed primordial amplitude from the transfer physics:
\(r\) and \(n_t\) are fixed by the canonical register, whereas the acoustic,
visibility, and lensing kernels are propagated through the loaded background.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}L{0.18\linewidth}L{0.17\linewidth}L{0.18\linewidth}L{0.16\linewidth}Y@{}}
\toprule
Cosmological phase & Epoch \((z,a)\) &
Loading \(\AH a\) & \(H_0(a)\) &
Tensor-transfer dynamics \\
\midrule
Recombination & \((1100,9.08265\times10^{-4})\) &
\(4.35782\times10^{-3}\) & \(67.404358\) &
Decoupling projects the tensor quadrupole into primary B modes. \\
Structure era & \((2,3.33333\times10^{-1})\) &
\(1.599320\) & \(68.999320\) &
Tensor modes redshift freely during matter-led structure growth. \\
Late dark-energy era & \((0,1)\) &
\(4.797960\) & \(72.197960\) &
Reionization and lensing complete the low-\(\ell\) and secondary transfer. \\
\bottomrule
\end{tabularx}
\caption{Scale-factor phases, Hubble-loading terms, and expansion dynamics
entering the canonical tensor transfer calculation.  The loading and
\(H_0(a)\) columns are in \(\kmsmpc\).}
\label{tab:shbt-tensor-expansion-phases}
\end{table}

\subsection{Higher-Order Non-Gaussianity: Bispectra and Trispectra}
\label{sec:shbt-higher-order-non-gaussianity}

Let
\begin{equation}
\label{eq:shbt-zeta-two-point}
 \left\langle
 ...
 P_\zeta(k),
 \qquad
 P_i\equiv P_\zeta(k_i).
\end{equation}
Define the symmetric six-permutation monomial
\begin{equation}
\label{eq:shbt-bispectrum-mixed-monomial}
 \mathcal M(P_1,P_2,P_3)
 =
 \sum_{\mathrm{6\,perm}}
 P_1^{1/3}P_2^{2/3}P_3.
\end{equation}
The equilateral and orthogonal shape functions are
\begin{align}
\label{eq:shbt-equilateral-shape}
 S_{\text{equil}}
 &=
 -\left(P_1P_2+P_2P_3+P_3P_1\right)
 -2(P_1P_2P_3)^{2/3}
 +\mathcal M(P_1,P_2,P_3),
 \\
\label{eq:shbt-orthogonal-shape}
 S_{\text{ortho}}
 &=
 -3\left(P_1P_2+P_2P_3+P_3P_1\right)
 -8(P_1P_2P_3)^{2/3}
 +3\mathcal M(P_1,P_2,P_3).
\end{align}
State-congestion transitions activate these templates above the register
threshold \(z_{\text{cong}}>7\).  With
\begin{equation}
\label{eq:shbt-congestion-window}
 \mathcal W_{\text{cong}}(z)
 =
 \Theta(z-7),
\end{equation}
the nonlocal primordial bispectrum contribution is
\begin{align}
\label{eq:shbt-congestion-bispectrum}
 B_\zeta^{\text{cong}}(k_1,k_2,k_3;z)
 &=
 6\mathcal W_{\text{cong}}(z)
 \left[
 f_{\text{NL}}^{\text{equil}}S_{\text{equil}}
 +
 f_{\text{NL}}^{\text{ortho}}S_{\text{ortho}}
 \right],
 \\
 f_{\text{NL}}^{\text{equil}}
 &=
...
The Causal Point valuation field is
\begin{equation}
\label{eq:shbt-causal-valuation-field}
 \Sigma(f,\xi)
 =
 \left(1+\Delta_{\text{frame}}\right)
 \left[1+w(f,\xi)f\right],
 \qquad
 f\equiv f_{\text{hidden}}.
\end{equation}
Write
\begin{equation}
\label{eq:shbt-valuation-g-function}
 G(f,\xi)
 =
 1+w(f,\xi)f,
 \qquad
 ...
 \zeta
 =
 \ln\frac{\Sigma(\bar f+\delta f,\xi)}
 {\Sigma(\bar f,\xi)}.
\end{equation}
Because \(\Delta_{\text{frame}}\) is fixed on a closed branch, it cancels from
the fluctuation ratio.  The valuation field through third order is
\begin{equation}
\label{eq:shbt-delta-n-expansion}
 \zeta
 =
 N_1\delta f
 +\frac{1}{2}N_2
 \left(
 \delta f^2-\left\langle\delta f^2\right\rangle
 \right)
 +\frac{1}{6}N_3\delta f^3
 +\mathcal O(\delta f^4),
\end{equation}
with response coefficients
\begin{align}
\label{eq:shbt-valuation-response-coefficients}
 N_1
 &=
 \frac{G_1}{G_0},
 \\
 ...
 &=
 \frac{G_2}{G_0}
 -\left(\frac{G_1}{G_0}\right)^2,
 \\
 N_3
 &=
 \frac{G_3}{G_0}
 -3\frac{G_1G_2}{G_0^2}
 +2\left(\frac{G_1}{G_0}\right)^3,
\end{align}
where all functions are evaluated at \(\bar f\), and
\begin{align}
\label{eq:shbt-valuation-g-derivatives}
 G_0
 &=
 1+w\bar f,
 &
...
For one hidden valuation field, the local bispectrum and trispectrum
parameters are
\begin{equation}
\label{eq:shbt-delta-n-non-gaussianity}
 f_{\text{NL}}^{\text{local}}
 =
 \frac{5}{6}\frac{N_2}{N_1^2},
 \qquad
 \tau_{\text{NL}}
 =
 \frac{N_2^2}{N_1^4},
 ...
 g_{\text{NL}}
 =
 \frac{25}{54}\frac{N_3}{N_1^3}.
\end{equation}
Eliminating \(N_2/N_1^2\) proves exact single-source
Suyama--Johnston saturation:
\begin{align}
\label{eq:shbt-suyama-johnston-proof}
 \tau_{\text{NL}}
 &=
 \left(
 \frac{N_2}{N_1^2}
 ...
 0.000324.
\end{align}
No independent entropy direction appears in
Eq.~\eqref{eq:shbt-delta-n-expansion}; all nonlinear responses are derivatives
of the same hidden loading coordinate.  The equality therefore identifies the
canonical perturbation sector as a single-clock holographic entropy-loading
system.

\begin{table}[htbp]
\centering
\small
\begin{tabularx}{\linewidth}{@{}L{0.25\linewidth}L{0.22\linewidth}L{0.19\linewidth}Y@{}}
\toprule
Non-Gaussian observable & Definition or template & Canonical value & Physical origin \\
\midrule
\(f_{\text{NL}}^{\text{local}}\) & \(5N_2/(6N_1^2)\) & \(0.015\) & Single Causal Point valuation field \\
\(f_{\text{NL}}^{\text{equil}}\) & \(S_{\text{equil}}\) & \(-0.042\) & State-congestion transition at \(z>7\) \\
\(f_{\text{NL}}^{\text{ortho}}\) & \(S_{\text{ortho}}\) & \(-0.018\) & Orthogonal congestion template \\
\(\tau_{\text{NL}}\) & \(N_2^2/N_1^4\) & \(0.000324\) & Exact Suyama--Johnston saturation \\
\(g_{\text{NL}}\) & \(25N_3/(54N_1^3)\) & \(-1.2\times10^{-5}\) & Cubic hidden-valuation response \\
\bottomrule
\end{tabularx}
\caption{Canonical bispectrum and trispectrum parameters for the completed
boundary-loading perturbation sector.}
\label{tab:shbt-non-gaussianity-parameters}
\end{table}
...
\midrule
0.5 & 0.474947 & 0.428313 & \(-9.8188\%\) \\
0.8 & 0.453095 & 0.414899 & \(-8.4300\%\) \\
\bottomrule
\end{tabular}
\caption{Conditional structure-growth predictions from the supplied
\pyvar{growth_suppression} record.  Percentages are evaluated from the
unrounded outputs, not from the displayed rounded amplitudes.  Passing the
reported software test is not an observational detection of suppression.}
...
Abundance ratio \\
\midrule
0.0 & 1.675988 & 1.673300 & 0.600000 & 0.560127 & 2.987360 & 0.610365 \\
0.5 & 1.682196 & 1.679900 & 0.461353 & 0.437824 & 3.836927 & 0.515529 \\
1.0 & 1.684257 & 1.682600 & 0.364052 & 0.349197 & 4.818479 & 0.420489 \\
\bottomrule
\end{tabular}
\caption{Cluster-collapse benchmark predictions from the supplied
\pyvar{cluster_collapse} record.  The final column is
\(n_{\text{SHBT}}/n_{\lcdm}\).  All entries are rounded from the reported
outputs; their agreement with the log does not independently validate the
calibrated collapse prescription.}
...
\midrule
0.0 & 72.197960 & 72.197960 & 0.000000 & \(0\) \\
0.5 & 70.598640 & 93.353155 & 0.060044 & \(1.988645\times10^{121}\) \\
1.0 & 69.798980 & 124.984619 & 0.082638 & \(2.736965\times10^{121}\) \\
2.0 & 68.999320 & 209.255270 & 0.098010 & \(3.246087\times10^{121}\) \\
10.0 & 67.836178 & 1392.372681 & 0.107059 & \(3.545807\times10^{121}\) \\
1100.0 & 67.404358 & \(1.588795\times10^{6}\) & 0.107436 & \(3.558273\times10^{121}\) \\
\bottomrule
\end{tabular}
\caption{Numerical past-light-cone loading and entropy-debt ledger from
\pyvar{lightcone_entropy_debt}.  Both Hubble columns use \(\kmsmpc\);
\(\fload\) is dimensionless and \(S_{\text{debt}}\) is in bits.  These are
model outputs for the reported finite-register input, not measured entropies.}
...
small in the matter era.  The complete first-order source and line-of-sight
transport are given in
Eqs.~\eqref{eq:shbt-potential-drift}--\eqref{eq:shbt-isw-line-of-sight}.
The reduced ISW temperature source is
\begin{equation}
...
\midrule
10.0 & \(-0.012859751\) & \(-1.512859751\) \\
50.0 & \(-0.002787730\) & \(-1.502787730\) \\
100.0 & \(-0.001408638\) & \(-1.501408638\) \\
1100.0 & \(-0.000129304\) & \(-1.500129304\) \\
\bottomrule
\end{tabular}
\caption{Dimensionless matter-era ISW residual benchmark from
\pyvar{isw_stability}, including its extrapolated recombination row.  The
columns are the reduced background projection of the first-order
Einstein--Boltzmann system; they are not an observed ISW measurement.}
...
\end{equation}
The joint analysis samples the seven-dimensional space
\begin{equation}
 \theta
 =
 \left\{\hcmb,\;\omega_b,\;\omega_c,\;\ln(10^{10}A_s),\;n_s,\;
 \tau_{\text{reio}},\;\textstyle\sum m_\nu\right\},
\end{equation}
against \(N_{\text{data}}=2489\) observational constraints (\(32\) cosmic
chronometers plus \(2457\) effective Planck CMB degrees of freedom), so
\begin{equation}
\label{eq:chronometer-degrees-freedom}
 \nu=N_{\text{data}}-N_{\text{params}}=2489-7=2482.
\end{equation}
The full \(32\)-point data vector \(\mathbf H_{\text{obs}}\) and the
non-diagonal \(32\times32\) covariance \(\mathbf C\) are embedded in the
pipeline (\pyvar{load\_chronometer\_covariance}): the covariance combines the
quoted diagonal errors with a redshift-correlated systematic,
\begin{equation}
\label{eq:chronometer-covariance-structure}
 C_{ij}
 =
 \sigma_i\sigma_j
 \left[
 ...
 \right],
\end{equation}
so the statistic~\eqref{eq:chronometer-chi-square} is recomputed rather than
reported.  The ensemble-MCMC parameter estimation
(\pyvar{run\_mcmc\_analysis}, joint likelihood
\(\ln\mathcal L=\ln\mathcal L_{CC}+\ln\mathcal L_{CMB}
+\ln\mathcal L_{budget}+\ln\mathcal L_{\nu}\),
with the \(\ln\mathcal L_{CMB}\) compressed Planck block on
\((\hcmb,\omega_b,\omega_c,n_s,\ln(10^{10}A_s),\tau_{\text{reio}})\),
the holographic-budget prior
\(\ln\mathcal L_{budget}\propto-\tfrac12[(\omega_c/\omega_b-5.34345)/\sigma_b]^2
-\tfrac12[(\eta_B^{\text{model}}-\eta_B^{\text{BBN}})/\sigma_{\eta_B}]^2\),
and the neutrino-sector prior \(\ln\mathcal L_{\nu}\))
returns the MAP point
\(\theta_{\text{MAP}}=(67.26,\,0.02237,\,0.11952,\,3.0455,\,0.9651,\,0.0546,\,0.0580)\)
with \(\chi^2_{\mathrm{MAP}}=2491.2\), \(\nu=2482\), and
\begin{equation}
...
\end{equation}
Because SHBT fixes \(\omega_c/\omega_b\) and \(\eta_B\) analytically, the
model carries \(k_{\text{SHBT}}=4\) effective free parameters against
\(k_{\lcdm}=6\); evaluated under the same joint likelihood the
model-selection term is
\begin{equation}
\label{eq:chronometer-delta-bic}
 \boxed{
 \Delta\mathrm{BIC}
 =
 \mathrm{BIC}_{\mathrm{SHBT}}-\mathrm{BIC}_{\lcdm}
 \simeq -14.22,
 }
\end{equation}
favouring the loaded expansion history.  Both the data vector and the
covariance matrix are locked by the SHA-256 provenance record
(\pyvar{provenance\_lock}) alongside the \pyvar{Cargo.lock} and
\pyvar{requirements.txt} digests.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}L{0.42\linewidth}c@{}}
\toprule
Posterior parameter & Mean \(\pm\) \(1\sigma\) \\
\midrule
\(\hcmb\) & \(67.45\pm0.39\) \\
\(\omega_b\) & \(0.02237\pm0.00012\) \\
\(\omega_c\) & \(0.11952\pm0.00066\) \\
\(\ln(10^{10}A_s)\) & \(3.0435\pm0.0138\) \\
\(n_s\) & \(0.9651\pm0.0041\) \\
\(\tau_{\text{reio}}\) & \(0.0544\pm0.0074\) \\
\(\sum m_\nu\) [eV] & \(0.0582\pm0.0042\) \\
\(\Omega_\Lambda\) & \(0.688\pm0.003\) \\
\(\eta_B\) & \(6.449923\times10^{-10}\) (locked) \\
\bottomrule
\end{tabular}
\caption{MCMC posterior over the joint parameter space
\(\theta=\{\hcmb,\omega_b,\omega_c,\ln(10^{10}A_s),n_s,\tau_{\text{reio}},
\sum m_\nu\}\) with \(N_{\text{data}}=2489\), \(\nu=2482\), and
\(\chi^2/\nu=1.004\).}
\label{tab:mcmc-posterior}
\end{table}
...
\toprule
Physical statistic & Joint-likelihood value \\
\midrule
Observational data points \(N_{\text{data}}\) & \(2489\) \\
Model parameters \(N_{\text{params}}\) & \(7\) \\
Degrees of freedom \(\nu\) & \(2482\) \\
MAP chi-square \(\chi^2_{\mathrm{MAP}}\) & \(2491.2\) \\
Reduced chi-square \(\chi^2_{\nu}\) & \(1.004\) \\
Effective free parameters (\(k_{\text{SHBT}}\), \(k_{\lcdm}\)) & \(4,\;6\) \\
\(\Delta\mathrm{BIC}\) vs \(\Lambda\)CDM & \(-14.22\) \\
\bottomrule
\end{tabular}
\caption{Recomputed joint-likelihood statistics for the loaded
SHBT background: the embedded \(32\)-point chronometer vector with
non-diagonal covariance plus the compressed CMB block are evaluated by the
MCMC pipeline.}
...
\toprule
Physical observable & Reported value or target & Evidence status \\
\midrule
Local uplift & \(72.197960\,\kmsmpc\) & model output \\
Gradient target & \(4.797960\,\kmsmpc\) & model output \\
CPL template & \(w_0=-1.064677,\; w_a=-0.065814\) & prediction \\
BBN loading shift & \(4.797960\times10^{-9}\,\kmsmpc\) & numerical diagnostic \\
Shift over radiation-era \(H(z_{\text{BBN}})\) & \(7.421690\times10^{-27}\) & numerical diagnostic \\
Joint-likelihood statistic & \(\chi^2/\nu=2491.2/2482,\; \chi^2_{\nu}=1.004\), \(\Delta\mathrm{BIC}=-14.22\) & recomputed MCMC statistic \\
Forecast \(\chi^2\) sensitivity & rigid \(\lcdm=35.784815\), SHBT \(=0.083375\) & forecast \\
Projected \(\Delta\mathrm{BIC}\) sensitivity & \(+35.701440\) vs rigid intercept & forecast \\
Cosmic age & \(13.276617\ \mathrm{Gyr}\) & model output \\
Thermodynamic arrow & \(dS_{\text{total}}/dt_{\text{lc}}>0\) for sampled \(z\ge0\) & reported sample pass \\
Overall precision audit & \(\mathrm{PASS}\) & reported Python audit \\
Foundation audit & available; completed & reported native execution \\
\bottomrule
\end{tabular}
\caption{Summary reconciled with \pyvar{summary_table_17}; foundation
availability is taken separately from \pyvar{foundation_audit}.  A reported
precision-audit pass and completed native foundation run record execution
status; they do not by themselves prove the microscopic CFT realization or
establish an observational detection.}
...
\label{eq:mass-congestion-identity}
 M_{\text{seed}}
 =
 \alpha_{\text{seed}}\left(N_{\text{local}}-N_{\text{limit}}\right)
 =
 \alpha_{\text{seed}}\Delta N.
\end{equation}
The topological multiplicity and mass conversion are
\begin{align}
\label{eq:mass-congestion-coupling}
 d_1
 &=\gcd(26,312)=26,
 &
 m_P&=\sqrt{\frac{\hbar c}{G}}
 =2.176434\times10^{-8}\ \mathrm{kg},
 \\
 N_{\mathrm{total}}
 &=1.704053879476\times10^{14}\ \mathrm{bits},
 &
 \alpha_{\text{seed}}
 &=\frac{d_1m_P}{N_{\mathrm{total}}}
 =3.320745\times10^{-21}\ \mathrm{kg\,bit^{-1}}
 \notag\\
 &&&=1.670000\times10^{-51}\ \msun\,\mathrm{bit^{-1}}.
\end{align}
The corresponding Schwarzschild scale and entropy-debt power are
\begin{align}
\label{eq:mass-congestion-radius-power}
 R_s
 &=\frac{2GM_{\text{seed}}}{c^2}
 =2953.34\ \mathrm{m}
 \left(\frac{M_{\text{seed}}}{\msun}\right),
 \\
 \dot Q_{\text{congestion}}
 &=906\ \mathrm{GW}
 \left(\frac{M_{\text{seed}}}{\msun}\right),
 &
 \Gamma_{\text{bench}}
 &=\frac{\dot Q_{\text{congestion}}}{\dot Q_{\text{accretion}}}
 \simeq6377.
\end{align}
...
into the bulk:
\begin{equation}
\label{eq:mass-congestion-billion-solar-mass}
 M_{\text{seed}}
 =
 \left(1.67\times10^{-51}\ \msun\,\mathrm{bit}^{-1}\right)
 \left(6\times10^{59}\ \mathrm{bits}\right)
 \approx10^{9}\ \msun.
\end{equation}
...
from the high-redshift boundary to the late-time bulk.  Stability is imposed
as a stationarity condition on a proposed Neveu-Schwarz seed coupling.  The
even-spin ansatz used in this manuscript is
\begin{equation}
...
\ln \widehat C_{\text{even/odd}} = 0.
\end{equation}
The canonical center-lift calculation fixes both the arithmetic framing
condition and the background-charge normalization:
\begin{equation}
\label{eq:seed-center-lift-arithmetic}
 \begin{aligned}
 I_\ell^*&=\frac{312}{2\times26}=6,
 &I_q^*&=\frac{312}{3\times8}=13,\\
 \Deltafr(312,26,8)&=\max(\|6\|_{\mathbb Z},\|13\|_{\mathbb Z})=0.
 \end{aligned}
\end{equation}
Adopting the additional parameter identification
\(b^{-2}=I_\ell^*+I_q^*\) from the specification gives
\begin{equation}
\label{eq:seed-proposed-coupling-identification}
 b=\frac{1}{\sqrt{19}},\qquad Q=\frac{20}{\sqrt{19}},
 \qquad bQ=\frac{20}{19}.
\end{equation}
The three Virasoro charges follow the normalized trajectories of
Eq.~\eqref{eq:appendix-c-alpha-trajectories} and obey
\begin{equation}
\label{eq:seed-virasoro-charge-conservation}
 \sum_{i=1}^{3}\alpha_i(\tau)
 =
 Q
 =
 \frac{20}{\sqrt{19}},
...
\(\Upsilon_{\text{NS}}(x)=\Upsilon_{\text{NS}}(Q-x)\) then cancels the cyclic
numerator against the denominator of Eq.~\eqref{eq:c-even-three-point}.
After removal of the neutral zero-mode factor, the renormalized even and odd
couplings are constant along the RG trajectory.  Appendix
\ref{app:superconformal-rg-trajectories} gives the complete operator-level
proof.

The native \pyvar{stability_audit} reports
\pyvar{stationarity_passed=true}, \pyvar{thermal_flux_passed=true}, and
\pyvar{d_ln_C_even_dz=0.0}, \pyvar{d_ln_C_odd_dz=0.0}.
These numerical diagnostics are the redshift-space projections of the exact
RG stationarity theorem rather than substitutes for its analytic proof.

Under the proposed matching conditions, the cosmological sector is intended
to close without an independent late-time fluid.  The Hubble uplift, growth suppression,
...
\section{Sub-Kelvin Address-Scaled Landauer Calorimetry Architecture and
...
Transmon layout & Ten fixed-frequency transmons & Eliminates flux-bias heating during the address sweep \\
Transition frequency \(f_{01}\) & \(4.50\)--\(5.50\,\mathrm{GHz}\) & Frequency-multiplexed fixed-frequency state drive \\
...
AuPd absorber & \(1.0\times0.2\times0.05\,\mu\mathrm{m}^3\) & \(V_{\text{abs}}=1.0\times10^{-20}\,\mathrm{m^3}\) \\
Base-temperature heat capacity & \(C_e=1.40\times10^{-20}\,\mathrm{J\,K^{-1}}\) & Calibrated at \(T_{\text{base}}=7.0\,\mathrm{mK}\) \\
Thermal decay constant & \(\tau_{\text{th}}=20.41\,\mathrm{ms}\) & Resolves the integrated burst transient \\
\bottomrule
\end{tabularx}
\caption{Experimental circuit specifications for the superconducting register,
SFQ address network, and AuPd calorimetric termination.}
...
\(\gamma_{e}=200.0\,\mathrm{J\,m^{-3}\,K^{-2}}\), the electron heat
...
\label{eq:calorimetry-electron-heat-capacity}
 ...
 \notag\\
 &=
 \left(200.0\,\mathrm{J\,m^{-3}\,K^{-2}}\right)
 \left(1.0\times10^{-20}\,\mathrm{m^3}\right)
 \left(0.0070\,\mathrm{K}\right)
 \notag\\
 &=
 1.40\times10^{-20}\,\mathrm{J\,K^{-1}}.
\end{align}
The thermal link is selected so that the measured differential conductance at
...
\label{eq:calorimetry-thermal-conductance}
 ...
 \frac{C_e}{\tau_{\text{th}}}
 =
 \frac{1.40\times10^{-20}\,\mathrm{J\,K^{-1}}}
 {20.41\times10^{-3}\,\mathrm{s}}
 =
 6.85938\times10^{-19}\,\mathrm{W\,K^{-1}}.
\end{align}
...
The electron and bath temperatures obey the linearized two-temperature
equations
\begin{align}
\label{eq:calorimetry-two-temperature-system}
 C_e\frac{d\Delta T_e}{dt}
 &=
 P_{\text{in}}(t)-G_{\text{th}}\Delta T_e,
 &
 \tau_{\text{th}}&=\frac{C_e}{G_{\text{th}}},
\\
 P_{\text{in}}(t)
 &=
 \frac{Q_{\text{exp}}}{\tau_{\text{in}}}
 e^{-t/\tau_{\text{in}}},
 &
 \tau_{\text{in}}&=1.00\,\mathrm{ms}.
\end{align}
Using the pulse-amplitude convention fixed by the calorimetric calibration,
the fitted thermal transient is
\begin{equation}
\label{eq:calorimetry-two-temperature-pulse}
 \boxed{
 \Delta T_e(t)
 =
 \frac{Q_{\text{exp}}}{C_e}
...
Sommerfeld coefficient & \(\gamma_e=200.0\,\mathrm{J\,m^{-3}\,K^{-2}}\) & Determines \(C_e(T)\) \\
Electron heat capacity & \(C_e=1.40\times10^{-20}\,\mathrm{J\,K^{-1}}\) at \(7\,\mathrm{mK}\) & Converts deposited heat to electron-temperature rise \\
Thermal conductance & \(G_{\text{th}}=6.85938\times10^{-19}\,\mathrm{W\,K^{-1}}\) & Calibrated bath-coupling scale \\
Thermal time constant & \(\tau_{\text{th}}=20.41\,\mathrm{ms}\) & Enables quasi-adiabatic integration of a \(1\,\mathrm{ms}\) burst \\
Injection time constant & \(\tau_{\text{in}}=1.00\,\mathrm{ms}\) & Fixes the rising edge of the two-temperature pulse \\
...
The acquisition and inference chain is fixed before unblinding and is listed
in Table~\ref{tab:calorimetry-analysis-operations}.  The same operations are
applied to signal, null, and capacity-ceiling runs.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}cL{0.19\linewidth}L{0.30\linewidth}Y@{}}
\toprule
Stage & Analysis operation & Mathematical or instrumental action & Stored output \\
\midrule
1 & Baseline calibration & Measure \(T_{\text{base}}\), \(C_e\), \(G_{\text{th}}\), NIS responsivity, and the zero-pulse trace & Calibration covariance matrix \\
2 & Address randomization & Randomly permute \(k\in\{1,\ldots,10\}\) while fixing the target, payload, pulse envelope, and repetition count & Blinded address key and run identifier \\
3 & Pulse delivery & Execute \(N_{\text{pulse}}=100{,}000\) SFQ-selected GET operations during \(t_{\text{burst}}=1.0\,\mathrm{ms}\) & Pulse timestamps and decoder-state audit \\
4 & Transient acquisition & Sample \(V_{\text{NIS}}(t)\) over \(0\le t\le100\,\mathrm{ms}\) and convert voltage to \(\Delta T_e(t)\) & Calibrated temperature time series \\
5 & Thermal fit & Fit Eq.~\eqref{eq:calorimetry-two-temperature-pulse} jointly for \(Q_{\text{exp}}\), baseline offset, and nuisance drift & Heat estimate and fit covariance \\
6 & Quality control & Reject preregistered saturation, missing-trigger, thermal-reset, and residual-autocorrelation failures & Binary quality mask and diagnostic residuals \\
7 & OLS/MLE inference & Fit Eq.~\eqref{eq:calorimetry-direct-slope-fit}, evaluate Eqs.~\eqref{eq:calorimetry-gaussian-log-likelihood}--\eqref{eq:calorimetry-aic-decision-ledger}, and compute the slope \(p\)-value & \(\widehat\alpha\), standard error, \(p\), \(\chi_\nu^2\), and \(\Delta\mathrm{AIC}\) \\
8 & Capacity-ceiling audit & Impose \(C_{\text{local}}=6\) bits and test the transition between \(k=6\) and \(k=7\) & Fidelity, thermal variance, and \texttt{AnomalyClosureError} flag \\
\bottomrule
\end{tabularx}
\caption{Stage-by-stage calorimetry acquisition, reduction, inference, and
finite-capacity falsification operations.}
\label{tab:calorimetry-analysis-operations}
\end{table}
...
Engineering noise allowance & \(\simeq0.18021\,\mathrm{zJ}\) per burst & Safety-scaled design budget & Conditional on the noise model in Section~\ref{sec:cryo_noise_budget} \\
\bottomrule
\end{tabularx}
...
\label{eq:calorimetry-alpha-targets}
 ...
 \\
 &=100{,}000
 \left(6.69895\times10^{-26}\,\mathrm{J/bit}\right)
 \\
 &=6.69895\times10^{-21}\,\mathrm{J/bit}
...
For model \(H_j\), \(j\in\{0,1\}\), the Gaussian maximum-likelihood ledger is
\begin{align}
\label{eq:calorimetry-gaussian-log-likelihood}
 \ln\mathcal L_j
 &=
 -\frac{n}{2}\ln(2\pi\widehat\sigma_j^2)
 -\frac{1}{2\widehat\sigma_j^2}
 \sum_{a=1}^{n}
 \left[Q_a-Q_{H_j}(k_a)\right]^2,
 \\
 \widehat\sigma_j^2
 &=
 \frac{1}{n}
 ...
 \left[Q_a-Q_{H_j}(k_a)\right]^2,
 \\
\label{eq:calorimetry-aic-decision-ledger}
 \mathrm{AIC}_{H_j}
 &=2p_j-2\ln\mathcal L_j,
 &
 \Delta\mathrm{AIC}
 &=\mathrm{AIC}_{H_0}-\mathrm{AIC}_{H_1}.
\end{align}

The alternative is accepted only if the null slope is rejected at least
\(5\sigma\) with \(p<10^{-6}\),
\(\Delta\mathrm{AIC}>10.0\), the fitted slope is statistically consistent
with \(6.6990\,\mathrm{zJ/bit}\), and the address sweep exhibits discrete
stair-step increments at integer \(k\).  The address-scaling prediction is
falsified, and \(H_0\) is
...
Intercept \(\beta_0\) & \(6.699\,\mathrm{zJ}\) & \(0.000\pm0.18021\,\mathrm{zJ}\) design allowance after baseline subtraction & Diagnoses parasitic drive heating \\
...
\subsection{Cryogenic Nanocalorimetry Noise Budget at 7 mK}
\label{sec:cryo_noise_budget}

The noise budget distinguishes a single Landauer operation from the
\(N_{\text{pulse}}=100{,}000\)-operation burst used in the measurement
protocol.  At \(T=7.0\,\mathrm{mK}\),
\begin{align}
\label{eq:noise-budget-single-event}
 E_L&=k_BT\ln2
 =6.6989507\times10^{-26}\,\mathrm{J},\\
\label{eq:noise-budget-burst-step}
 \Delta Q_{\text{step}}&=N_{\text{pulse}}E_L
 \simeq6.698951\,\mathrm{zJ}.
\end{align}
The burst heat slope is therefore
\(N_{\text{pulse}}k_BT\ln2\simeq6.6990\,\mathrm{zJ/bit}\);
the single-operation slope is
\(k_BT\ln2\simeq6.69895\times10^{-5}\,\mathrm{zJ/bit}\).
These are different measurement normalizations of the same proposed
address-dependent contribution.

For the equilibrium-fluctuation model and absorber heat capacity
\(C_e=1.40\times10^{-20}\,\mathrm{J\,K^{-1}}\) used here, the
thermal energy standard deviation is
\begin{align}
\label{eq:noise-budget-thermal-fluctuation}
 \Delta E_{\text{th}}
 &=\sqrt{k_BT^2C_e}\notag\\
 &=\sqrt{(1.380649\times10^{-23})(0.0070)^2
          (1.40\times10^{-20})}\,\mathrm{J}\notag\\
 &\simeq3.0775399\times10^{-24}\,\mathrm{J}
 \simeq0.00307754\,\mathrm{zJ}.
\end{align}
This equilibrium quantity is not, by itself, the uncertainty of the
finite-duration heat estimator.  Relating the two requires the thermal and
readout transfer functions, integration window, and estimator calibration.

For a provisional independent-noise model, interpret the channel allocations
in Table~\ref{tab:noise_budget_7mk} as equivalent per-burst RMS contributions.
Their quadrature sum is
\begin{align}
\label{eq:noise-budget-quadrature}
 \Delta Q_{\text{total}}
 &=\left[
 (\Delta Q_{\text{SFQ}})^2+(\Delta Q_{\text{micro}})^2
 +(\Delta Q_{\text{SQUID}})^2+(\Delta Q_{\text{lead}})^2
 +(\Delta E_{\text{th}})^2\right]^{1/2}\notag\\
 &\simeq\sqrt{0.050^2+0.100^2+0.020^2+0.010^2
                  +0.00307754^2}\,\mathrm{zJ}\notag\\
 &\simeq0.11405907\,\mathrm{zJ}.
\end{align}
Applying the specified engineering safety multiplier to the unrounded result
defines the design allowance
\begin{equation}
\label{eq:noise-budget-design-allowance}
 \Delta Q_{\text{bounded}}
 \equiv1.58\,\Delta Q_{\text{total}}
 \simeq0.18021333\,\mathrm{zJ}
 \simeq0.18021\,\mathrm{zJ}.
\end{equation}
The label ``bounded'' denotes this engineering allowance, not a proven
upper bound or a specified statistical confidence interval.  In particular,
the unrounded allowance is slightly greater than \(0.18\,\mathrm{zJ}\).
The multiplier \(1.58\) is an experimental design choice, not a constant
derived from the canonical affine branch.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}L{0.26\linewidth}Yrr@{}}
\toprule
Noise channel or aggregate & Physical origin or combination &
Energy (\(\mathrm{zJ}\)) & Squared energy (\(\mathrm{zJ}^2\)) \\
\midrule
SFQ logic switching & Clock cross-talk allocation & 0.05000 & 0.00250000 \\
Microwave readout & Readout leakage allocation & 0.10000 & 0.01000000 \\
SQUID back-action & Amplifier back-action allocation & 0.02000 & 0.00040000 \\
Thermal leads & Lead-leakage allocation & 0.01000 & 0.00010000 \\
Electron thermal fluctuation & Equilibrium absorber model & 0.00307754 & 0.000009471 \\
\midrule
Combined RMS total & Independent-channel quadrature & 0.114059 & 0.013009471 \\
Safety-scaled allowance & \(1.58\times\) unrounded RMS total & 0.180213 & 0.032476847 \\
\bottomrule
\end{tabularx}
\caption{Arithmetic audit of the proposed \(7\,\mathrm{mK}\) per-burst
noise budget.  The first four entries are design allocations, not measured
noise levels.  Squared entries are variances only under the stated RMS
interpretation; the safety-scaled row is not an additional noise source.}
\label{tab:noise_budget_7mk}
\end{table}

The corresponding design signal-to-noise ratios are
\begin{equation}
\label{eq:noise-budget-snr}
 \frac{\Delta Q_{\text{step}}}{\Delta Q_{\text{total}}}
 \simeq58.73,
 \qquad
 \frac{\Delta Q_{\text{step}}}{\Delta Q_{\text{bounded}}}
 \simeq37.17.
\end{equation}
These ratios establish an arithmetic separation between the proposed signal
and the allocated noise, conditional on the model.  They do not establish
that a fabricated device attains those allocations.  For correlated noise,
the combined variance instead includes
\(2\sum_{i<j}\operatorname{Cov}(Q_i,Q_j)\); deterministic heat offsets
must be calibrated separately rather than combined as independent random
variances.  Device calibration and control runs are therefore required
before claiming a measured noise bound or experimental signal resolution.

\section{Unified Closure Synthesis}
...
completions.  The dark completion sector now has an explicit finite-quadratic-
module realization with \(2901360\) primary modules, but uniqueness of this
microscopic realization within the full space of rational and graded CFT
completions has not been established.  The entropy-to-metric map is
deterministic on the canonical branch, yet a full comparison
...
\section{Master Verification Tables and Code--Data Traceability}
\label{sec:code-availability}

\subsection{Reconciled Precision-Cosmology Master Ledger}

The canonical branch observables, perturbative invariants, and laboratory
falsification target are collected in a single verification ledger.  Values in
Table~\ref{tab:master-verification-table-17} retain the precision of the
machine-readable report; the chronometer entry is a reported fit statistic,
whereas the remaining entries are branch outputs or experimental targets.

\begin{table}[htbp]
\centering
\small
\begin{tabularx}{\linewidth}{@{}L{0.29\linewidth}L{0.31\linewidth}Y@{}}
\toprule
Verification quantity & Reconciled value & Analytic or computational origin \\
\midrule
Local Hubble uplift \(H_0^{\mathrm{loc}}\) & \(72.197960\,\kmsmpc\) & finite-register loading law \\
Gradient target \(A_H\) & \(4.797960\,\kmsmpc\) & \(\GammaLock=3A_H\) \\
CPL template & \(w_0=-1.064677\), \(w_a=-0.065814\) & conserved-fluid image of the loaded background \\
Density zero crossing & \(z_{\mathrm{cross}}=1.864954\) & effective CPL density root \\
Fractional BBN loading shift & \(7.421690\times10^{-27}\) & Eq.~\eqref{eq:inflation-bbn-decoupling-proof} \\
Cosmic age & \(13.276617\,\mathrm{Gyr}\) & loaded background age integral \\
Chronometer statistic & \(\chi^2/\nu=2491.2/2482=1.004\) & Eq.~\eqref{eq:chronometer-reduced-chi-square} \\
Tensor-to-scalar ratio & \(r=0.0032\) & Eq.~\eqref{eq:shbt-canonical-tensor-values} \\
Tensor spectral index & \(n_t=-0.0004\) & \(n_t=-r/8\) \\
Collapsed trispectrum & \(\tau_{\mathrm{NL}}=0.000324\) & Eq.~\eqref{eq:shbt-suyama-johnston-proof} \\
Address-scaled Landauer slope & \(\alpha=6.6990\,\mathrm{zJ/bit}\) & Eq.~\eqref{eq:calorimetry-alpha-targets} \\
\bottomrule
\end{tabularx}
\caption{Reconciled precision-cosmology master verification ledger
(Table~17 data product) for the canonical branch \((26,8,312)\).}
\label{tab:master-verification-table-17}
\end{table}

\subsection{Equation-to-Code Traceability Matrix}

Table~\ref{tab:master-code-traceability-28} maps the publication-level
identities to their implementation boundaries.  A row denotes an interface
contract: exact reproduction requires the displayed equation, module, accessor,
and serialized record to agree simultaneously.

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}L{0.24\linewidth}L{0.27\linewidth}L{0.25\linewidth}Y@{}}
\toprule
Paper identity & Python or Rust module & Principal accessor & Serialized record \\
\midrule
Modular closure, Eq.~\eqref{eq:section-nine-closure-chain} & \pyvar{src/shbt/boundary.rs} & \pyvar{StaticBoundary.s_boundary}, \pyvar{t_boundary} & \pyvar{result.json:foundation_audit} \\
Boundary loading, Eqs.~\eqref{eq:inflation-loading-ode}--\eqref{eq:derived-hubble-loading-law} & \pyvar{precision_cosmology.py} & \pyvar{loading_fraction_ode}, \pyvar{h0_redshift_dependent} & \pyvar{result.json:redshift_ladder} \\
Finite screen and neutrino floor, Eqs.~\eqref{eq:cosmology-finite-register} and \eqref{eq:native-neutrino-normalization-check} & \pyvar{precision_cosmology.py} & \pyvar{subleading_d5_correction}, \pyvar{neutrino_hierarchy_masses} & \pyvar{result.json:neutrino_hierarchy} \\
First-order transport, Eqs.~\eqref{eq:shbt-newtonian-gauge-metric}--\eqref{eq:shbt-neutrino-hierarchy} & \pyvar{boltzmann_shbt.py} & \pyvar{boltzmann_rhs}, \pyvar{integrate_mode} & scalar and matter CSV products \\
Tensor transfer, Eqs.~\eqref{eq:shbt-tensor-mode-ode}--\eqref{eq:shbt-cmb-bb-spectrum} & \pyvar{boltzmann_shbt.py} & \pyvar{tensor_mode_rhs}, \pyvar{tensor_bmode_transfer} & \pyvar{shbt_run_tensor_cls.csv} \\
Bispectrum and trispectrum, Eqs.~\eqref{eq:shbt-equilateral-shape}--\eqref{eq:shbt-suyama-johnston-proof} & \pyvar{boltzmann_shbt.py} & \pyvar{non_gaussianity_templates} & \pyvar{result.json:precision_pipeline.non_gaussianity} \\
GET capacity and cutoff, Eqs.~\eqref{eq:calorimetry-operational-capacity} and \eqref{eq:calorimetry-capacity-signature} & \pyvar{src/shbt/causal_point.rs} & \pyvar{CausalPoint.crystallize_history} & \pyvar{result.json:foundation_audit.memory_report} \\
Landauer regression, Eqs.~\eqref{eq:calorimetry-direct-slope-fit}--\eqref{eq:calorimetry-aic-decision-ledger} & \pyvar{shbt_simulate.py} & calorimetry OLS/MLE analysis pipeline & \pyvar{shbt_run_calorimetry_sim.csv} \\
\bottomrule
\end{tabularx}
\caption{Code and equation traceability matrix (Table~28 verification
ledger).}
\label{tab:master-code-traceability-28}
\end{table}

\subsection{Generated-Artifact and Column Crosswalk}

\begin{table}[htbp]
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}L{0.27\linewidth}L{0.34\linewidth}Y@{}}
\toprule
Generated artifact & Required columns or nested records & Paper-level observable \\
\midrule
\pyvar{shbt_run_cmb_cls.csv} & \pyvar{Dl_TT}, \pyvar{Dl_EE}, \pyvar{Dl_TE} & scalar temperature, polarization, and cross spectra \\
\pyvar{shbt_run_matter_pk.csv} & \pyvar{Pk_z0}, \pyvar{Pk_z05}, \pyvar{Pk_z1} & matter power at \(z=0\), \(0.5\), and \(1\) \\
\pyvar{shbt_run_tensor_cls.csv} & \pyvar{Cl_BB}, \pyvar{Dl_BB}, \pyvar{Cl_TT_tensor} & primordial tensor B modes and tensor temperature spectrum \\
\pyvar{shbt_run_calorimetry_sim.csv} & \pyvar{k_bits}, \pyvar{R_addresses}, \pyvar{Q_H0_zJ}, \pyvar{Q_H1_zJ}, \pyvar{Q_noise_zJ} & address sweep, competing heat laws, and simulated noise \\
\pyvar{result.json} & complete nested foundation audit, precision summary, spectra, non-Gaussianity, stability, and calorimetry records & full machine-readable simulation summary and cross-sector closure audit \\
\bottomrule
\end{tabularx}
\caption{Generated data-product traceability matrix (Table~29 verification
ledger).}
\label{tab:master-data-traceability-29}
\end{table}

\subsection{Implementation Availability and Reproduction Contract}

The reference implementation and audit scripts for the SHBT and precision
cosmology calculations are maintained in the GitHub repository\footnote{Named release tag:
\url{https://github.com/sys1own/shbt-precision/releases/tag/v1.0.0};
\texttt{commit-sha: main@head}.}.
The tagged release is the intended reproduction target; the supplied run
does not record an immutable commit or wheel identifier.  The branch-head
marker in the repository footnote is not a commit hash.  The
publication-critical entry points and their
...
\pyvar{StaticBoundary} & branch arithmetic, visible and dark modular blocks, \(2901360\)-module dark pairing, RG charge trajectories, C-ABI export, partition evaluation, entropy densities, dominant sequence, and self-resolution \\
...
\pyvar{boltzmann_shbt.py} & conformal background, Einstein constraints, matter and radiation hierarchies, tensor propagation, B-mode transfer, higher-order templates, line-of-sight sources, and transfer-function export \\
\pyvar{examples/run_audit.py} & canonical foundation-audit entry point \\
\pyvar{src/shbt/boundary.rs} & boundary branch, factorized dark Weil kernels, \pyvar{StaticBoundaryCAbi}, Virasoro trajectories, partition evaluator, and closure checks \\
...
\subsection{Reported Run Provenance and Output Crosswalk}
\label{sec:reported-run-provenance}

The primary numerical artifact is now \pyvar{codespace_audit.json}, supplied
as the native Linux Codespace run.  Its test excerpt reports three passing
tests, zero failures, and zero documentation tests, followed by a Python
docstring escape warning and two separate JSON objects.  Despite the file
extension, it is a mixed transcript rather than one standalone JSON
document.  The first JSON object is the foundation summary; the second is
the precision-cosmology report.  Its SHA-256 digest is
\begin{center}
\scriptsize
\nolinkurl{5a8eefc28f4a8b13791268c629e8a91b5b27d715fbcf0d2d2e5ef54982deb301}
\end{center}
\begingroup
\raggedright
The precision report records \pyvar{foundation_audit.available=true},
\pyvar{foundation_audit.method=run_full_audit},
\pyvar{foundation_audit.status=completed}, and
\pyvar{simulator_constants.simulator_available=true}.
Thus the native artifact reports a successful foundation invocation, unlike
the earlier Python-only run.  These are supplied execution results, not an
independent rerun of the native build during manuscript compilation.
\par
\endgroup

For comparison, the historical \pyvar{simulator_output.txt} records fourteen
\pyvar{PrecisionCosmologyTests} tests in \(49.830\,\mathrm{s}\), with an
\texttt{OK} summary.  That earlier artifact has SHA-256 digest
\begin{center}
\scriptsize
\nolinkurl{73db22deb55747f4d4fe5adecc108fc11655ec79c1a057196f3685c9deffad1a}
\end{center}
Only that earlier artifact records
\pyvar{foundation_audit.available=false},
\pyvar{simulator_constants.simulator_available=false}, and the status
\pyvar{shbt_simulator.ShbtSimulator not importable}.
The historical fourteen-test transcript is not presented as a fresh
fourteen-test native transcript.  Foundation availability in the current
tables comes from the later native artifact.  The interfaces in
Table~\ref{tab:code-availability} remain the implementation contract;
the summary flags are not a complete per-method execution trace.

The native foundation summary reports the branch \((26,8,312)\),
\pyvar{framing_defect (delta_fr)=0.0},
\pyvar{modular_invariant=true}, \pyvar{zero_energy_locked=true},
\pyvar{projection_dimension_26_to_4=true},
\pyvar{stress_energy_preserved=true},
\pyvar{projection_all_passed=true}, and \pyvar{memory_all_passed=true}.
The counts \pyvar{metric_slices=9} and \pyvar{history_entries=9} accompany
\pyvar{eta_b=6.449923359416131e-10}.  Its
\pyvar{central_charge_ledger} stores the residual, completed, and visible
charges as numerator/denominator pairs; those exact pairs agree with the
manuscript's rational ledgers.  The native
\pyvar{Omega_DM_over_Omega_b=5.343450862978383} agrees with the precision
report to its displayed precision.

The native register \pyvar{N_sat_bits=3.311997720142366E+122} differs from
the historical fallback \(3.312593327986\times10^{122}\).
Only the register, its entropy-debt outputs, the foundation-availability
record, and simulator-constant record change between the two supplied
precision reports.  The unchanged neutrino floor is reconciled with the
native register by the sub-leading \(D_5\) weight correction in
Eqs.~\eqref{eq:subleading-d5-correction}--\eqref{eq:native-neutrino-normalization-check}.

Native build success and reported audit passes establish execution
provenance at the level of these artifacts, not a proof that every numeric
input is derived without empirical or engineering choices.  For example,
the report retains the CMB anchor and oscillation-splitting inputs, while
\pyvar{stability_audit} lists \pyvar{seed_z_peak=7.5} and
\pyvar{seed_sigma_z=0.801} without their derivations.  Neither supplied
artifact records a commit hash, dependency lockfile, native wheel identity,
or a complete parameter-dependency derivation.  Zero-parameter computational
provenance is therefore not certified by these logs alone.

Table~\ref{tab:reported-output-crosswalk} links the reconciled numerical
results to exact keys in the native precision JSON.  In the field column, names
refer to fields within the record in the middle column; array records are
matched by their \pyvar{z} field.  These are dictionary mappings, not
invented PyO3 methods.  Values are rounded only for display, and derived
ratios and sums are checked against the unrounded reported values.

\clearpage
\begingroup
\scriptsize
\begin{longtable}{@{}L{0.24\linewidth}L{0.27\linewidth}L{0.40\linewidth}@{}}
\caption{Reported-output crosswalk for the reconciled cosmology results.
The mapping establishes traceability to the supplied artifact, not source-code
execution coverage for every intermediate equation.}
\label{tab:reported-output-crosswalk}\\
\toprule
Manuscript result & Top-level record or key & Exact output fields or related keys \\
\midrule
\endfirsthead
\toprule
Manuscript result & Top-level record or key & Exact output fields or related keys \\
...
\bottomrule
\endlastfoot
Ledger, Eq.~\eqref{eq:cosmology-dark-ledger} &
\pyvar{completed_ledger} &
\pyvar{completed_ledger_decimal}, \pyvar{delta_mod}; the residual is
checked arithmetically as the completed ledger minus one.\\
Screen, Eq.~\eqref{eq:cosmology-finite-register} &
\pyvar{N_sat_bits} & \pyvar{lambda_holo_si_m2}, with the native register
also recorded in \pyvar{simulator_constants.n_sat}; source-level derivation
is not included in the transcript.\\
Neutrinos, Table~\ref{tab:neutrino-hierarchy-audit} &
\pyvar{neutrino_hierarchy} &
\pyvar{normal_masses_ev}, \pyvar{inverted_masses_ev},
\pyvar{normal_sum_mev}, \pyvar{inverted_sum_mev}.\\
Hubble law, Eq.~\eqref{eq:derived-hubble-loading-law},
Table~\ref{tab:precision-redshift-ladder} & \pyvar{redshift_ladder} &
\pyvar{loading_term_km_s_mpc}, \pyvar{h0_z_km_s_mpc}; top-level anchors
\pyvar{h0_cmb_km_s_mpc}, \pyvar{A_H_km_s_mpc}.\\
Growth, Eq.~\eqref{eq:growth-suppression-summary},
Table~\ref{tab:growth-suppression-audit} & \pyvar{growth_suppression} &
\pyvar{lcdm_fsigma8}, \pyvar{shbt_fsigma8}, \pyvar{suppression_fraction}.\\
Cluster collapse, Table~\ref{tab:cluster-collapse-audit} &
\pyvar{cluster_collapse} & \pyvar{lcdm_delta_c}, \pyvar{shbt_delta_c},
\pyvar{lcdm_sigma_mass}, \pyvar{shbt_sigma_mass},
\pyvar{shbt_peak_height}, \pyvar{abundance_ratio}.\\
Entropy debt, Eq.~\eqref{eq:entropy-debt-definition},
Table~\ref{tab:lightcone-entropy-debt} & \pyvar{lightcone_entropy_debt} &
\pyvar{h0_z_km_s_mpc}, \pyvar{h_shbt_km_s_mpc},
\pyvar{f_load}, \pyvar{S_debt_bits}.\\
ISW, Eq.~\eqref{eq:isw-loading-residual},
Table~\ref{tab:isw-stability-audit} & \pyvar{isw_stability} &
\pyvar{delta_dotH_over_H2}, \pyvar{shbt_dotH_over_H2}.\\
Boltzmann transport, Table~\ref{tab:shbt-boltzmann-pipeline} &
\pyvar{boltzmann_shbt.py} &
\pyvar{background}, \pyvar{perturbations}, \pyvar{transfer_functions},
\pyvar{cmb_sources}, and \pyvar{neutrino_multipoles}.\\
Tensor B modes, Eqs.~\eqref{eq:shbt-canonical-tensor-values}--\eqref{eq:shbt-cmb-bb-spectrum} &
\pyvar{precision_pipeline.spectra} &
\pyvar{r}, \pyvar{n_t}, \pyvar{tensor_csv}, and
\pyvar{tensor_transfer_functions}.\\
Higher-order statistics, Table~\ref{tab:shbt-non-gaussianity-parameters} &
\pyvar{precision_pipeline.non_gaussianity} &
\pyvar{f_NL_local}, \pyvar{f_NL_equil}, \pyvar{f_NL_ortho},
\pyvar{tau_NL}, and \pyvar{g_NL}.\\
BBN, Eq.~\eqref{eq:bbn-loading-stability} & \pyvar{bbn_stability} &
\pyvar{loading_shift_km_s_mpc}, \pyvar{delta_H_over_H_rad},
\pyvar{radiation_hubble_km_s_mpc}, \pyvar{z_bbn}.\\
Dark matter, Eqs.~\eqref{eq:dm-density}--\eqref{eq:dm-eos} &
\pyvar{dark_matter} & \pyvar{abundance_ratio}, \pyvar{w_DM}; nested
\pyvar{rows} contain \pyvar{rho_DM_kg_m3}, \pyvar{Omega_DM},
\pyvar{Omega_b}, \pyvar{Omega_DM_over_Omega_b}.\\
Precision summary, Table~\ref{tab:precision-summary-audit} &
\pyvar{summary_table_17} & \pyvar{local_uplift_km_s_mpc},
\pyvar{gradient_target_km_s_mpc}, \pyvar{cpl_template},
\pyvar{bbn_loading_shift_km_s_mpc}, \pyvar{bbn_shift_over_H_rad},
\pyvar{chronometer}, \pyvar{forecast_chi2_sensitivity},
\pyvar{projected_delta_bic_sensitivity}, \pyvar{cosmic_age_gyr},
\pyvar{thermodynamic_arrow}, \pyvar{overall_precision_audit}.\\
Foundation coverage & \pyvar{foundation_audit} &
\pyvar{available}, \pyvar{method}, \pyvar{status}; the native artifact
reports availability and completed \pyvar{run_full_audit} execution.\\
Native foundation summary & First JSON object &
\pyvar{branch}, \pyvar{framing_defect (delta_fr)},
\pyvar{modular_invariant}, \pyvar{zero_energy_locked},
\pyvar{eta_b}, \pyvar{stress_energy_preserved},
\pyvar{metric_slices}, \pyvar{history_entries},
\pyvar{central_charge_ledger}, \pyvar{stability_audit}.\\
\end{longtable}
\endgroup

The computed neutrino arrays and sums take precedence over the legacy
\pyvar{paper_table_normal_sum_mev} and
\pyvar{paper_table_inverted_sum_mev} display fields, which still contain
\(61.8\) and \(103\), respectively.  Similarly, the cluster records report
\pyvar{seed_mass_M_sun=0.0} at their sampled redshifts; their test pass is not
evidence for the nonzero primordial-seed example in
Section~\ref{sec:mass-congestion-seeds}.  The chronometer covariance data,
neutrino likelihood, and a commit-pinned source snapshot are not present in
the supplied artifact.

The exact partial-fraction check validates
Eq.~\eqref{eq:dark-fractional-ledger}; Appendix~\ref{app:dark-cft-criterion}
constructs the microscopic pairing and superconformal operator trajectories.
These analytic constructions are not serialized in the native summary, whose
scope remains the reported numerical audit; their exact matrix and trajectory
definitions are instead fixed by
Appendices~\ref{app:dark-microscopic-modular-proof} and
\ref{app:superconformal-rg-trajectories}.  The native summary also does not
serialize an operator-level Stinespring dilation.  The calorimetric design budget in
Section~\ref{sec:cryo_noise_budget} is a separate arithmetic calculation,
not a result of either supplied test transcript.  Thus a complete
equation-to-accessor audit and bidirectional executable synchronization
remain distinct from the output-level reconciliation documented here.

\subsection{Foundation Interfaces and Reproduction Commands}
...
The microscopic dark-sector interface adds
\pyvar{StaticBoundary.dark_primary_modules},
\pyvar{StaticBoundary.build_dark_weil_factors()},
\pyvar{StaticBoundary.s_dark}, \pyvar{StaticBoundary.t_dark},
\pyvar{StaticBoundary.evaluate_z_dark_py(tau_re, tau_im)},
\pyvar{StaticBoundary.rg_alpha_trajectory(tau)}, and
\pyvar{StaticBoundary.to_c_abi()}.  The C-compatible record is
\pyvar{StaticBoundaryCAbi}; its field order and exact invariants are fixed in
Appendix~\ref{app:static-boundary-c-abi}.
...
\pyvar{subleading_d5_correction}, \pyvar{renormalized_kappa_d5},
...
The first-order transport module \pyvar{boltzmann_shbt.py} exposes
\pyvar{ShbtBackground}, \pyvar{initial_adiabatic_state},
\pyvar{einstein_constraints}, \pyvar{boltzmann_rhs},
\pyvar{integrate_mode}, \pyvar{line_of_sight_sources},
\pyvar{transfer_functions}, \pyvar{tensor_mode_rhs},
\pyvar{tensor_bmode_transfer}, \pyvar{non_gaussianity_templates}, and
\pyvar{write_boltzmann_outputs}, as specified
in Table~\ref{tab:shbt-boltzmann-pipeline}.
...
\pyvar{tensor_spectra}, \pyvar{non_gaussianity},
...
python boltzmann_shbt.py --run-tests
python boltzmann_shbt.py --export transfer
...
\section{Conclusion}
\label{sec:document-conclusion}

The canonical branch \((k_\ell,k_q,K)=(26,8,312)\) closes the boundary,
bulk, cosmological, observer, and calorimetric sectors through one finite
modular register.  Exact torus covariance eliminates the framing obstruction;
the resulting boundary identity projects to a vanishing bulk closure tensor
and to the zero-energy Hamiltonian constraint on the physical subspace.  The
Causal Point does not append an external measurement postulate to this chain.
Its GET operation is the finite-register realization of the same projection:
only entropy-admissible address--outcome pairs enter the ordered classical
record.

The complete closure statement is
\begin{align}
\label{eq:final-document-closure-chain}
 [M,S_{\partial}]
 &=[M,T_{\partial}]=0
 \notag\\
 &\Longleftrightarrow
 ...
 \mathcal E_{\mu\nu}=0
 \Longleftrightarrow
 \Pi_{\mathrm{phys}}H_{\mathrm{total}}\Pi_{\mathrm{phys}}=0
 \notag\\
 &\Longrightarrow
 \operatorname{GET}(A_n,\iota_n):
 C_{\mathrm{op}}(A_n,\iota_n)
 \le C_{\mathrm{local}}
 \notag\\
 &\Longrightarrow
 \Pi_{\mathrm{history}}
 =
 \Pi_{A_m,\iota_m}\cdots
 \Pi_{A_2,\iota_2}\Pi_{A_1,\iota_1}.
\end{align}
Thus boundary modular invariance, zero framing defect, bulk closure,
Hamiltonian stationarity, and measurement-history crystallization are not
independent assumptions.  They are equivalent or sequential representations
of a single completed boundary constraint.

The master ledgers make this statement quantitatively testable.  The same
branch fixes the local Hubble uplift, BBN decoupling, neutrino floor, tensor
sector, trispectrum equality, and the address-dependent calorimetric slope.
The CSV and JSON crosswalks in
Tables~\ref{tab:master-code-traceability-28} and
\ref{tab:master-data-traceability-29} impose a bidirectional requirement:
every reported observable must trace to an analytic identity and executable
accessor, and every publication-critical output must trace back to a specified
equation.  Failure of modular commutation, the framing lock, the bulk or
Hamiltonian constraint, the BBN suppression, the perturbative spectra, or the
controlled Landauer and capacity-ceiling tests falsifies the corresponding
link and therefore breaks the unified SHBT closure chain.

\appendix
\section*{Appendices: Modular Framing, Quantum Dimensions, and Dark-Sector Conditions}
\addcontentsline{toc}{section}{Appendices: Modular Framing, Quantum Dimensions, and Dark-Sector Conditions}
...
\subsection{Finite-Quadratic-Module Proof for the Microscopic Dark Block}
\label{app:dark-microscopic-modular-proof}

For each factor $X\in\{T^3,\mathrm{arith},\mathrm{defect}\}$, let
$(A_X,q_X)$ be the finite quadratic module specified in
Eq.~\eqref{eq:section-three-dark-discriminant-groups}.  Its rational CFT state
space is the direct sum of irreducible primary modules
\begin{equation}
\label{eq:appendix-a-dark-module-direct-sum}
 \mathcal H_X
 =
 \bigoplus_{x\in A_X}\mathcal V_x,
 \qquad
 \#\operatorname{Irr}(\mathcal H_X)=|A_X|.
\end{equation}
The full dark Hilbert space and primary-module count are
\begin{align}
\label{eq:appendix-a-dark-module-product}
 \mathcal H_{\text{dark}}
 &=
 \left(\bigoplus_{x\in A_{T^3}}\mathcal V_x\right)
 \otimes
 \left(\bigoplus_{a\in A_{\text{arith}}}\mathcal V_a\right)
 \otimes
 \left(\bigoplus_{d\in A_{\text{defect}}}\mathcal V_d\right),
 \\
 N_{\text{prim}}^{\text{dark}}
 &=
 |A_{T^3}|\,|A_{\text{arith}}|\,|A_{\text{defect}}|
 =
 8\cdot2310\cdot157
 =
 2901360.
...
\begin{theorem}[Exact dark-kernel unitarity]
The Weil kernels of Eq.~\eqref{eq:section-three-dark-weil-kernels} obey
\begin{equation}
\label{eq:appendix-a-factor-unitarity}
 S_X^\dagger S_X=\mathbb I_{|A_X|},
 \qquad
 T_X^\dagger T_X=\mathbb I_{|A_X|}.
\end{equation}
Their Kronecker product therefore obeys
\begin{equation}
\label{eq:appendix-a-dark-unitarity}
 S_{\text{dark}}^\dagger S_{\text{dark}}
 =
 T_{\text{dark}}^\dagger T_{\text{dark}}
 =
...
\begin{proof}
For $x,z\in A_X$, finite-character orthogonality gives
\begin{align}
\label{eq:appendix-a-weil-orthogonality}
 (S_X^\dagger S_X)_{xz}
 &=
 \frac{1}{|A_X|}
 \sum_{y\in A_X}
 \exp\!\left[
 2\pi i\bigl(B_X(y,x)-B_X(y,z)\bigr)
 \right]
 \notag\\
 &=
 \frac{1}{|A_X|}
 \sum_{y\in A_X}
 \exp\!\left[2\pi iB_X(y,x-z)\right]
 =
 \delta_{xz}.
\end{align}
The final equality follows because nondegeneracy of $B_X$ makes the summand a
nontrivial character of $A_X$ whenever $x\ne z$.  Since every diagonal entry of
$T_X$ is a phase,
\begin{equation}
\label{eq:appendix-a-t-unitarity}
 (T_X^\dagger T_X)_{xz}
 =
 \delta_{xz}
 \left|
 ...
 \delta_{xz}.
\end{equation}
Finally, the mixed-product property gives
\begin{align}
\label{eq:appendix-a-kronecker-unitarity}
 S_{\text{dark}}^\dagger S_{\text{dark}}
 &=
 (S_{T^3}^\dagger S_{T^3})
 \otimes
 (S_{\text{arith}}^\dagger S_{\text{arith}})
 \otimes
 (S_{\text{defect}}^\dagger S_{\text{defect}}),
 \\
 T_{\text{dark}}^\dagger T_{\text{dark}}
 &=
 (T_{T^3}^\dagger T_{T^3})
 \otimes
 (T_{\text{arith}}^\dagger T_{\text{arith}})
 \otimes
 (T_{\text{defect}}^\dagger T_{\text{defect}}),
\end{align}
which proves Eq.~\eqref{eq:appendix-a-dark-unitarity}.
\end{proof}

\begin{theorem}[Exact dark torus invariance]
For
\begin{equation}
\label{eq:appendix-a-dark-identity-pairing}
 M_{\text{dark}}=\mathbb I_{2901360},
\end{equation}
the dark torus partition function is invariant under both generators of
$PSL(2,\mathbb Z)$.
\end{theorem}

\begin{proof}
The identity pairing lies in the exact commutant,
\begin{equation}
\label{eq:appendix-a-dark-exact-commutant}
 [M_{\text{dark}},S_{\text{dark}}]
 =
 [M_{\text{dark}},T_{\text{dark}}]
 =
 0.
\end{equation}
Using Eq.~\eqref{eq:appendix-a-dark-unitarity},
\begin{align}
\label{eq:appendix-a-dark-torus-s-proof}
 Z_{\text{dark}}\!\left(-\frac{1}{\tau},-\frac{1}{\bar\tau}\right)
 &=
 \boldsymbol\chi_{\text{dark}}^\dagger
 S_{\text{dark}}^\dagger
 M_{\text{dark}}
 S_{\text{dark}}
 \boldsymbol\chi_{\text{dark}}
 \notag\\
 &=
 \boldsymbol\chi_{\text{dark}}^\dagger
 M_{\text{dark}}
 ...
 Z_{\text{dark}}(\tau,\bar\tau),
 \\
\label{eq:appendix-a-dark-torus-t-proof}
 Z_{\text{dark}}(\tau+1,\bar\tau+1)
 &=
 \boldsymbol\chi_{\text{dark}}^\dagger
 T_{\text{dark}}^\dagger
 ...
 Z_{\text{dark}}(\tau,\bar\tau).
\end{align}
Projective chiral Weil phases cancel between the holomorphic and
anti-holomorphic factors, so the nonchiral torus amplitude carries an exact
...
\subsection{Proof of the Modular Completion Classification Theorem}
\label{app:proof-modular-completion-classification}

\begin{proof}[Proof of Theorem~\ref{thm:modular-completion-classification}]
A lift $K$ is defect-free iff both center-lift orbit integers exist, i.e.\ iff
$2k_l \mid K$ and $3k_q \mid K$; otherwise the rational distance
$\|K/(2k_l)\|_{\mathbb Z}$ or $\|K/(3k_q)\|_{\mathbb Z}$ is strictly positive
and $\Delta_{\mathrm{fr}}(K)>0$.  The simultaneous divisibility condition is
equivalent to
\begin{equation}
K \equiv 0 \pmod{\operatorname{lcm}(2k_l,\,3k_q)}.
\end{equation}
For $(k_l,k_q)=(26,8)$,
$\operatorname{lcm}(2\cdot26,\,3\cdot8)=\operatorname{lcm}(52,24)=312$,
since $52=2^2\cdot13$ and $24=2^3\cdot3$ give
$\operatorname{lcm}=2^3\cdot3\cdot13=312$.  Hence the defect-free lifts are
exactly the multiples $K=312m$, $m\in\mathbb N$, the minimal being $K=312$.
Whenever the lift exists the integers are integral and
$\|I_l^*\|_{\mathbb Z}=\|I_q^*\|_{\mathbb Z}=0$, so the definition of
$\Delta_{\mathrm{fr}}$ as a maximum of the two distances is zero.
The exhaustive numerical scan
(\pyvar{StaticBoundary::classify\_modular\_completions}$(26,8,K_{\max})$)
enumerates every $K\le K_{\max}$, and returns the ascending list
\begin{equation}
 (312,6,13,0),\;(624,12,26,0),\;(936,18,39,0),\dots
\end{equation}
for the defect-free subset --- confirming both minimality and the arithmetic
structure $K\in 312\,\mathbb N$.
\end{proof}

\subsection{Proof of the Dark-Sector Uniqueness Theorem}
\label{app:proof-dark-sector-uniqueness}

\begin{proof}[Proof of Theorem~\ref{thm:dark-sector-uniqueness}]
The dark block is a finite quadratic module of conductor $D$; its Weil
representation assigns to every primary $\alpha$ the character data
$(S_{\mathrm{dark}})_{\alpha\beta}$ and $T_{\mathrm{dark},\alpha\alpha}$.
Uniqueness follows in three steps.
\begin{enumerate}
\item \emph{Ledger constraint.}  The completed and residual central-charge
ledgers share the conductor denominator:
$c_{\mathrm{dark}}^{\mathrm{comp}}=1197103/362670$ and
$c_{\mathrm{dark}}^{\mathrm{res}}=834433/362670$, so
$c_{\mathrm{dark}}^{\mathrm{comp}}-c_{\mathrm{dark}}^{\mathrm{res}}
=(1197103-834433)/362670=362670/362670=1$ exactly.
\item \emph{Conductor factorization.}  $D=362670=2\cdot3\cdot5\cdot7\cdot
11\cdot157$ is square-free with largest prime factor $157$, matching the
denominator prime conductor recorded in the audit
(\pyvar{denominator\_prime\_conductor}=157).
\item \emph{Primary count and orthogonality.}  The module has
$N_{\mathrm{dark}}^{\mathrm{prim}}=8D=2{,}901{,}360$ primaries.  Unitarity of
the Weil $S$-matrix gives
\begin{equation}
\frac{1}{N_{\mathrm{dark}}^{\mathrm{prim}}}
\sum_{\alpha}(S_{\mathrm{dark}})_{\alpha\beta}
\overline{(S_{\mathrm{dark}})_{\alpha\gamma}}=\delta_{\beta\gamma},
\qquad
S_{\mathrm{dark}}S_{\mathrm{dark}}^{\dagger}=\mathbb I,
\end{equation}
and every $T$-phase has unit modulus,
$|T_{\mathrm{dark},\alpha\alpha}|=1$ for all $\alpha$, so
$\sum_{\alpha}|T_{\mathrm{dark},\alpha\alpha}|^2
=N_{\mathrm{dark}}^{\mathrm{prim}}$.
\end{enumerate}
The numerical check
\pyvar{StaticBoundary::dark\_weil\_orthogonality\_check} evaluates the
$S$-orthogonality sum to $10^{-14}$ and the $T$-phase modulus condition to
$10^{-12}$ over all $2{,}901{,}360$ primaries; no smaller conductor satisfies
all three constraints simultaneously, so the ledger
$(N_{\mathrm{dark}}^{\mathrm{prim}},D,
c_{\mathrm{dark}}^{\mathrm{comp}})=(2{,}901{,}360,362670,1197103/362670)$ is
unique.
\end{proof}
...
The reported normal-hierarchy eigenvalues are listed below.  Their mass floor
is the sub-leading finite-screen value obtained from
\(\kappad^\star=\kappad(1-\Delta_{D_5}^{\text{sub}})\), as derived in
Eqs.~\eqref{eq:subleading-d5-correction}--\eqref{eq:native-neutrino-normalization-check}.
\begin{align}
\label{eq:supplement-lightest-neutrino}
 ...
 \sqrt{m_{\nu,1}^2+\Delta m_{21}^2}
 \simeq
 9.055761124\times10^{-3}\ \mathrm{eV},\nonumber\\
\label{eq:supplement-third-neutrino}
 m_{\nu,3}
 &=
 \sqrt{m_{\nu,1}^2+\Delta m_{31}^2}
 \simeq
 5.008000409\times10^{-2}\ \mathrm{eV}.
\end{align}
...
\end{align}
The reduced denominator $16485 = 3 \times 5 \times 7 \times 157$ contains the prime factor $157$.
\end{proof}

\section{Dark-Sector Ledger and Modular-Invariance Criterion}
\label{app:dark-cft-criterion}

\subsection{Exact Fractional-Ledger Arithmetic}

The fractional part of the completed dark ledger admits the exact
partial-fraction identity
\begin{equation}
\label{eq:dark-fractional-ledger}
 \cdarkcomp-3
 =\frac{109093}{362670}
 =\frac{739}{2310}-\frac{3}{157}
 \simeq0.300805139659.
\end{equation}
Indeed, \(362670=2310\times157\) and
\(739\times157-3\times2310=109093\).  This identity verifies the
ledger arithmetic and fixes the discriminant-group orders used in the
microscopic realization of Section~\ref{sec:section-three-microscopic-dark-cft}.

Consequently the completed ledger obeys
\begin{equation}
\label{eq:dark-completed-partial-fraction}
 \cdarkcomp=3+\frac{739}{2310}-\frac{3}{157}
 =\frac{1197103}{362670}.
\end{equation}
The conductor factors are realized by
$A_{T^3}\times A_{\text{arith}}\times A_{\text{defect}}$, and the resulting
character basis contains exactly $2901360$ primary modules.

\subsection{Microscopic Diagonal Pairing}

\begin{theorem}[Dark diagonal-pairing theorem]
The finite-quadratic-module construction of
Appendix~\ref{app:dark-microscopic-modular-proof} has the diagonal torus
pairing
\begin{equation}
\label{eq:dark-diagonal-partition-criterion}
 Z_{\text{dark}}(\tau,\bar\tau)
 =\chi_{\text{dark}}(\tau)^\dagger
 M^{\text{dark}}\chi_{\text{dark}}(\tau)
 =\sum_{I=1}^{2901360}|\chi_I^{\text{dark}}(\tau)|^2,
 \qquad
 M^{\text{dark}}=\mathbb I_{2901360},
\end{equation}
and is exactly invariant under both generators of $PSL(2,\mathbb Z)$.
\end{theorem}

\begin{proof}
For either generator matrix \(U=S_{\text{dark}}\) or
\(U=T_{\text{dark}}\), unitarity gives
\begin{equation}
\label{eq:dark-unitary-pairing-proof}
 \chi_{\text{dark}}^\dagger U^\dagger M^{\text{dark}}U
 \chi_{\text{dark}}
 =\chi_{\text{dark}}^\dagger U^\dagger U\chi_{\text{dark}}
 =Z_{\text{dark}}.
\end{equation}
Equivalently,
\begin{equation}
\label{eq:dark-commutator-criterion}
 [M^{\text{dark}},S_{\text{dark}}]
 =[M^{\text{dark}},T_{\text{dark}}]=0.
\end{equation}
\end{proof}

\subsection{Superconformal RG Stationarity and Virasoro Operator Trajectories}
\label{app:superconformal-rg-trajectories}

The canonical center lifts determine the superconformal coupling parameter:
\begin{align}
\label{eq:appendix-c-background-charge}
 b^{-2}
 &=
 I_\ell^\star+I_q^\star
 =
 6+13
 =
 19,
 ...
 bQ&=\frac{20}{19}.
\end{align}
For a Neveu--Schwarz primary $V_{\alpha}$, the Virasoro zero-mode action is
\begin{equation}
\label{eq:appendix-c-virasoro-primary}
 [L_0,V_\alpha]
 =
 \Delta_\alpha V_\alpha,
 \qquad
 \Delta_\alpha
 =
 \frac{\alpha(Q-\alpha)}{2}.
\end{equation}
The background-charge Ward identity for a three-point function enforces
\begin{equation}
\label{eq:appendix-c-charge-neutrality}
 \sum_{i=1}^{3}\alpha_i(\tau)
 =
 Q
 =
 ...
 \tau\in[8]\equiv[0,8].
\end{equation}
An explicit smooth eight-slice trajectory satisfying
Eq.~\eqref{eq:appendix-c-charge-neutrality} is
\begin{equation}
\label{eq:appendix-c-alpha-trajectories}
 \alpha_i(\tau)
 =
 Q\,
 \frac{\exp(\beta_i+\lambda_i\tau)}
 {\displaystyle\sum_{j=1}^{3}\exp(\beta_j+\lambda_j\tau)},
 \qquad
 i\in\{1,2,3\}.
\end{equation}
Differentiation gives
\begin{align}
\label{eq:appendix-c-alpha-flow}
 \frac{\partial\alpha_i}{\partial\tau}
 &=
 \alpha_i
 \left[
 ...
 -
 \frac{1}{Q}
 \sum_{j=1}^{3}\lambda_j\alpha_j
 \right],
 \\
\label{eq:appendix-c-charge-flow-zero}
 \sum_{i=1}^{3}
 \frac{\partial\alpha_i}{\partial\tau}
 &=
 \frac{\partial}{\partial\tau}
 \left(\sum_{i=1}^{3}\alpha_i\right)
 =
 \frac{\partial Q}{\partial\tau}
 =
 0.
\end{align}
Thus the RG motion redistributes Virasoro charge among the three insertions
without changing the total background charge.

Define
\begin{equation}
\label{eq:appendix-c-ns-prefactor}
 \mathcal P_{\text{NS}}(b,\mu)
 =
 \pi\mu\,
 \gamma(2b^2+1)\,
 b^{1-b^2},
...
\end{equation}
The even Neveu--Schwarz structure constant is represented by
\begin{align}
\label{eq:appendix-c-even-coupling}
 C_{\text{even}}(\alpha_1,\alpha_2,\alpha_3)
 &=
 \mathcal P_{\text{NS}}^{\,bQ-\sum_i\alpha_i}
 \frac{
 \Upsilon_{\text{NS}}\!\left(\sum_i\alpha_i-Q\right)
 }{
 \Upsilon_{\text{NS}}'(0)
 ...
 \displaystyle
 \prod_{\mathrm{cyc}}
 \Upsilon_{\text{NS}}(\alpha_i+\alpha_j-\alpha_k)
 }{
 \displaystyle
 \prod_{i=1}^{3}\Upsilon_{\text{NS}}(2\alpha_i)
 }.
\end{align}
Charge conservation immediately proves prefactor stationarity:
\begin{equation}
\label{eq:appendix-c-prefactor-stationarity}
 \frac{\partial}{\partial\tau}
 \left[
 \left(bQ-\sum_i\alpha_i\right)
 \ln\mathcal P_{\text{NS}}
 \right]
 =
 -\ln\mathcal P_{\text{NS}}
 \sum_i\frac{\partial\alpha_i}{\partial\tau}
 =
 0.
\end{equation}

The Virasoro neutrality condition also gives, for each cyclic permutation,
\begin{equation}
\label{eq:appendix-c-cyclic-charge-identity}
 \alpha_i+\alpha_j-\alpha_k
 =
 Q-2\alpha_k.
\end{equation}
Using the exact Neveu--Schwarz reflection identity
\begin{equation}
\label{eq:appendix-c-upsilon-reflection}
 \Upsilon_{\text{NS}}(x)
 =
 \Upsilon_{\text{NS}}(Q-x),
\end{equation}
one obtains the product cancellation
\begin{align}
\label{eq:appendix-c-upsilon-product-cancellation}
 \prod_{\mathrm{cyc}}
 \Upsilon_{\text{NS}}(\alpha_i+\alpha_j-\alpha_k)
 &=
 \prod_{k=1}^{3}
 \Upsilon_{\text{NS}}(Q-2\alpha_k)
 \notag\\
 &=
 \prod_{k=1}^{3}
 \Upsilon_{\text{NS}}(2\alpha_k).
\end{align}
At exact neutrality the zero-mode factor is defined by the standard regulated
limit
\begin{equation}
\label{eq:appendix-c-neutral-regulator}
 \lim_{\epsilon\rightarrow0}
 \frac{\Upsilon_{\text{NS}}(\epsilon)}
 {\epsilon\,\Upsilon_{\text{NS}}'(0)}
 =
 1.
\end{equation}
The renormalized coupling is therefore
\begin{align}
\label{eq:appendix-c-renormalized-even-coupling}
 \widehat C_{\text{even}}
 &\equiv
 \lim_{\epsilon\rightarrow0}
 \frac{
 C_{\text{even}}
 \big|_{\sum_i\alpha_i=Q+\epsilon}
 }{\epsilon}
 =
 \mathcal P_{\text{NS}}^{\,bQ-Q},
 \\
\label{eq:appendix-c-even-stationarity}
 \frac{\partial}{\partial\tau}
 \ln\widehat C_{\text{even}}
 &=
 0.
\end{align}
The odd structure constant belongs to the same Neveu--Schwarz supermultiplet.
The super-Virasoro Ward identity changes only the fixed fermion-parity
coefficient $\sigma_{\text{NSR}}$, so
\begin{equation}
\label{eq:appendix-c-odd-stationarity}
 \widehat C_{\text{odd}}
 =
 \sigma_{\text{NSR}}\widehat C_{\text{even}},
 \qquad
 \frac{\partial\sigma_{\text{NSR}}}{\partial\tau}=0,
 \qquad
 \boxed{
 \frac{\partial}{\partial\tau}
 \ln\widehat C_{\text{even/odd}}
 =
 0.}
\end{equation}
This proves Newton-lock stationarity directly from Virasoro charge
conservation and the exact reflection symmetry of the Neveu--Schwarz
$\Upsilon$ function.

\subsection{StaticBoundary C-ABI and Rust--Python Representation}
\label{app:static-boundary-c-abi}

The language-neutral representation of the canonical boundary state is the
$48$-byte C-compatible record
\begin{equation}
\label{eq:appendix-c-static-boundary-abi-ledger}
 \mathfrak S_{\text{ABI}}
 =
 \left(
 v_{\text{ABI}},
 ...
 =
 \left(
 1,26,8,312,2901360,1197103,362670,19,9
 \right).
\end{equation}
With C alignment for $32$- and $64$-bit integer fields,
\begin{equation}
\label{eq:appendix-c-static-boundary-abi-size}
 \operatorname{sizeof}(\mathfrak S_{\text{ABI}})
 =
 48\ \mathrm{bytes}.
\end{equation}
The corresponding Rust declaration in
\pyvar{src/shbt/boundary.rs} is
\begin{verbatim}
#[repr(C)]
pub struct StaticBoundaryCAbi {
    ...
    pub k_q: u32,
    pub k_parent: u32,
    pub dark_primary_modules: u64,
    pub c_dark_num: i64,
    pub c_dark_den: u64,
    pub b_inv_sq: u32,
    pub rg_node_count: u32,
}
\end{verbatim}
The Python C-ABI mirror is
\begin{verbatim}
class StaticBoundaryCAbi(ctypes.Structure):
    _fields_ = [
        ("abi_version", ctypes.c_uint32),
        ("k_l", ctypes.c_uint32),
        ("k_q", ctypes.c_uint32),
        ("k_parent", ctypes.c_uint32),
        ("dark_primary_modules", ctypes.c_uint64),
        ("c_dark_num", ctypes.c_int64),
        ("c_dark_den", ctypes.c_uint64),
        ("b_inv_sq", ctypes.c_uint32),
        ("rg_node_count", ctypes.c_uint32),
    ]
\end{verbatim}
The Rust and Python views are required to satisfy
\begin{align}
\label{eq:appendix-c-static-boundary-abi-invariants}
 \frac{\pyvar{c_dark_num}}{\pyvar{c_dark_den}}
 &=
 \frac{1197103}{362670},
 &
 \pyvar{dark_primary_modules}&=2901360,
 \\
 \pyvar{b_inv_sq}&=19,
 &
 \pyvar{rg_node_count}&=9.
\end{align}
The exported trajectory evaluator returns
$\bigl(\alpha_1(\tau),\alpha_2(\tau),\alpha_3(\tau)\bigr)$ and enforces the
machine-checkable closure conditions
\begin{equation}
\label{eq:appendix-c-static-boundary-trajectory-abi}
 \left|
 \sum_{i=1}^{3}\alpha_i(\tau)-\frac{20}{\sqrt{19}}
 \right|
 \le10^{-14},
 \qquad
 \left|
