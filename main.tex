...
accounting under explicit hypotheses; a thermodynamic immunity proof still
requires the missing interaction dynamics.
...
value throughout.  Its effect on the separate neutrino normalization is
checked in Eq.~\eqref{eq:native-neutrino-normalization-check}.
...
Neutrino floor & $m_{\nu,1}^{\text{out}}$ & $2.829630635353\times10^{-3}\ \mathrm{eV}$ & reported; normalization check pending \\
...
The native register and reported neutrino floor must be distinguished:
Eq.~\eqref{eq:native-neutrino-normalization-check} gives their normalization
discrepancy with the stated constants.}
\label{tab:noether-bridge-audit}
\end{table}

\subsection{Neutrino Hierarchy Lock}

The proposed finite-register matching relation assigns a neutrino floor to
the quarter-root register scale weighted by the retained \(D_5\)
...
\end{align}
The native report nevertheless retains the same lightest mass as the earlier
Python-only report.  With no rescaling of \(\kappad\) or \(M_P\), the
two values differ:
\begin{equation}
\label{eq:native-neutrino-normalization-check}
 ...
 \frac{m_{\nu,1}^{\text{reg}}}{m_{\nu,1}^{\text{out}}}-1
 &\simeq4.4955321\times10^{-5}.
 \end{aligned}
\end{equation}
The earlier register value reproduces the reported floor to the precision
of the quoted constants, but the native substitution does not.  The
\(0.00449553\%\) discrepancy is an unresolved normalization check, not
a reason to introduce a fitted compensating factor.  The eigenstates and
sums below reproduce the native output: in these formulas,
\(m_{\nu,1}\) denotes \(m_{\nu,1}^{\text{out}}\).
...
columns reproduce the reported diagnostic, rather than a full radiation-era
transfer-function calculation or an observed ISW measurement.}
...
the supplied report records \(\chi^2=30.16\), \(\nu=29\), and
\(\chi^2_\nu=1.04\).  The observational vector and covariance matrix are
not included in that artifact, so this is a reported statistic rather than
an independently recomputed fit.  Its arithmetic normalization is
\begin{equation}
...
\bottomrule
\end{tabular}
\caption{Reported cosmic-chronometer benchmark statistics for the loaded
SHBT background.  Independent likelihood validation requires the underlying
...
\end{equation}
If Eq.~\eqref{eq:newton-lock-stationarity} is established for a specified
operator family, it fixes the seed coupling along the flow.  The canonical
center-lift calculation itself establishes only the arithmetic framing
condition:
\begin{equation}
...
\label{eq:seed-proposed-coupling-identification}
 ...
 \qquad bQ=\frac{20}{19}.
\end{equation}
This identification is an additional model prescription.  The integer-lift
arithmetic does not specify the individual \(\alpha_i\), impose
\(\sum_i\alpha_i=20/19\), or determine their RG derivatives.  Those
operator constraints are needed before a vanishing exponent or full
coupling stationarity can be inferred.
...
These are successful reported benchmark diagnostics with respect to
redshift.  The transcript does not supply the differentiated functions,
operator trajectories, or their relation to the RG parameter \(\tau\).
It therefore does not turn the center-lift arithmetic into a first-principles
proof of Eq.~\eqref{eq:newton-lock-stationarity}.
...
Engineering noise allowance & \(\simeq0.18017\,\mathrm{zJ}\) per burst & Safety-scaled design budget & Conditional on the noise model in Section~\ref{sec:cryo_noise_budget} \\
\bottomrule
\end{tabularx}
...
Intercept \(\beta_0\) & \(6.699\,\mathrm{zJ}\) & \(0.000\pm0.18017\,\mathrm{zJ}\) design allowance after baseline subtraction & Diagnoses parasitic drive heating \\
...
\(C_e=4.90\times10^{-21}\,\mathrm{J\,K^{-1}}\) used here, the
...
\label{eq:noise-budget-thermal-fluctuation}
 ...
 &=\sqrt{(1.380649\times10^{-23})(0.0070)^2
          (4.90\times10^{-21})}\,\mathrm{J}\notag\\
 &\simeq1.8206972\times10^{-24}\,\mathrm{J}
 \simeq0.0018207\,\mathrm{zJ}.
\end{align}
...
\label{eq:noise-budget-quadrature}
 ...
 &\simeq\sqrt{0.050^2+0.100^2+0.020^2+0.010^2
                  +0.0018207^2}\,\mathrm{zJ}\notag\\
 &\simeq0.11403208\,\mathrm{zJ}.
\end{align}
...
\label{eq:noise-budget-design-allowance}
 ...
 \simeq0.18017068\,\mathrm{zJ}
 \simeq0.18017\,\mathrm{zJ}.
\end{equation}
...
Electron thermal fluctuation & Equilibrium absorber model & 0.0018207 & 0.000003315 \\
\midrule
Combined RMS total & Independent-channel quadrature & 0.114032 & 0.013003315 \\
Safety-scaled allowance & \(1.58\times\) unrounded RMS total & 0.180171 & 0.032461476 \\
\bottomrule
\end{tabularx}
...
precision reports.  The unchanged neutrino floor is therefore reported
separately from its native-register recomputation in
...
separates this arithmetic proof from a conditional modular-pairing criterion.
The native summary does not supply a dark-sector
character basis or modular matrices, an operator-level Stinespring
construction, or the missing link between integer center lifts and
superconformal RG stationarity.  No output-to-method assignment is claimed
for those missing calculations.  The calorimetric design budget in
...
The reported normal-hierarchy eigenvalues are listed below.  They retain the
native report's mass floor; the distinct mass obtained from the native
register substitution is compared in
Eq.~\eqref{eq:native-neutrino-normalization-check}.
\begin{align}
...
ledger arithmetic; it does not specify a coset embedding, a character basis,
or a modular-invariant microscopic CFT realization.
...
\label{eq:dark-completed-partial-fraction}
 ...
 =\frac{1197103}{362670}.
\end{equation}
The native denominator diagnostic records the prime \(157\).  Establishing
the conductor of a particular modular representation additionally requires
its explicit modular data; the rational ledger alone does not supply them.

\subsection{Conditional Diagonal-Pairing Criterion}

\begin{theorem}[Diagonal-pairing criterion]
Suppose a specified dark CFT carrying the required completed central charge
has a finite character vector \(\chi_{\text{dark}}\) closed under unitary
modular matrices \(S_{\text{dark}}\) and \(T_{\text{dark}}\).  Then its
diagonal character pairing
\begin{equation}
\label{eq:dark-diagonal-partition-criterion}
 Z_{\text{dark}}(\tau,\bar\tau)
 =\chi_{\text{dark}}(\tau)^\dagger
 M^{\text{dark}}\chi_{\text{dark}}(\tau)
 =\sum_i|\chi_i^{\text{dark}}(\tau)|^2,
 \qquad M^{\text{dark}}=\mathbb I,
\end{equation}
is invariant under both modular generators.
\end{theorem}
...
This proves the pairing criterion under its stated hypotheses, not the
existence of the required microscopic CFT.  A constructive completion must
still provide the character spectrum, conformal weights, and modular
matrices of a theory realizing the ledger.  Neither a matching central
charge nor a reported modular-invariance flag supplies that construction.
